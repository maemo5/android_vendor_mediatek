...
int bias_atp[2] = {0,0};
int matrix_atp[8] = {0, 0, 0, 0, 0, 0, 0, 0};

int g_pass_case_atp1 =0;
int g_pass_case_atp2 =0;
int g_pass_case_atp3 =0;
int g_pass_case_atp4 =0;
int g_pass_case_atp5 =0;

int tangle_atp_px1 =0;
int tangle_atp_py1 =0;
int tangle_atp_px2 =0;
int tangle_atp_py2 =0;
int tangle_atp_px3 =0;
int tangle_atp_py3 =0;
int tangle_atp_px4 =0;
int tangle_atp_py4 =0;
...
// for ATA test
int g_detected_atp = 0;

void clear_touch_temp_data_atp(void)
{
    tangle_atp_px1 =0;
    tangle_atp_py1 =0;
    tangle_atp_px2 =0;
...
void clear_touch_result_atp(void)
{
    g_pass_case_atp1 =0;
    g_pass_case_atp2 =0;
    g_pass_case_atp3 =0;
...
void paint_sensitivity_atp(struct touch *tpd) {
...
static void detect_points(struct touch *tpd, int x, int y)
{
	...
	if (TOUCH_L1 > y && g_pass_case_atp1 == 0)		// up side
	{
	...
	if (TOUCH_L1 > (tpd->width - x) && g_pass_case_atp1 == 0)		// right side
	{
		update_px = tpd->width;
	...
	if ((TOUCH_L1 > (tpd->height - y)) && g_pass_case_atp1 == 0)		//down side
	{
		update_px = (x/TOUCH_L1) * TOUCH_L1;
	...
	if (TOUCH_L1 > x && g_pass_case_atp1 == 0)		//left side
	{
		update_px = 0;
		update_py = (y/TOUCH_L1) * TOUCH_L1;
		update_x = 0;
	...
	if ( g_pass_case_atp1 == 1 && g_pass_case_atp2 == 0 && g_pass_case_atp3 == 0)
	{
...
static void paint_ctpTest(struct touch *tpd) 
{
    ...
    ui_color(255,0,0,255);
    {
		...
		if(0==g_pass_case_atp1 && 0==g_pass_case_atp2 && 0==g_pass_case_atp3 && 0==g_pass_case_atp4 && 0==g_pass_case_atp5 )
		{
		     // draw the tangle
...
/* Vanzo:songlixin on: Tue, 15 Sep 2015 14:33:13 +0800
 */
            if (get_is_ata())
            {
              int h1 = (tpd->height - 4 * TOUCH_L1) / 7;
		      ui_color(255,0,0,255);
              ui_fill(2 * TOUCH_L1, 2 * TOUCH_L1, tpd->width - 2 * TOUCH_L1, 2 * TOUCH_L1 + h1);
		      ui_color(0,255,0,255);
              ui_fill(2 * TOUCH_L1, 2 * TOUCH_L1 + h1, tpd->width - 2 * TOUCH_L1, 2 * TOUCH_L1 + h1 * 2);
		      ui_color(0,0,255,255);
              ui_fill(2 * TOUCH_L1, 2 * TOUCH_L1 + h1 * 2, tpd->width - 2 * TOUCH_L1, 2 * TOUCH_L1 + h1 * 3);
		      ui_color(255,255,0,255);
              ui_fill(2 * TOUCH_L1, 2 * TOUCH_L1 + h1 * 3, tpd->width - 2 * TOUCH_L1, 2 * TOUCH_L1 + h1 * 4);
		      ui_color(0,255,255,255);
              ui_fill(2 * TOUCH_L1, 2 * TOUCH_L1 + h1 * 4, tpd->width - 2 * TOUCH_L1, 2 * TOUCH_L1 + h1 * 5);
		      ui_color(255,0, 255,255);
              ui_fill(2 * TOUCH_L1, 2 * TOUCH_L1 + h1 * 5, tpd->width - 2 * TOUCH_L1, 2 * TOUCH_L1 + h1 * 6);
		      ui_color(255,255,255,255);
              ui_fill(2 * TOUCH_L1, 2 * TOUCH_L1 + h1 * 6, tpd->width - 2 * TOUCH_L1, 2 * TOUCH_L1 + h1 * 7);
            }
// End of Vanzo:songlixin
		}
		if(1==g_pass_case_atp1 && 0==g_pass_case_atp2 && 0==g_pass_case_atp3 && 0==g_pass_case_atp4 && 0==g_pass_case_atp5 )
		{
		  // draw corner
		...
		if(1==g_pass_case_atp1 && 1==g_pass_case_atp2 && 1==g_pass_case_atp3 && 0==g_pass_case_atp4 && 0==g_pass_case_atp5 )
		{
		  LOGD(TAG " 2..................................");
		  ui_color(255,0,0,255);
		...
		if(1==g_pass_case_atp1 && 1==g_pass_case_atp2 && 1==g_pass_case_atp3 && 1==g_pass_case_atp4 && 0== g_pass_case_atp5 )
		{
		  LOGD(TAG " 3..................................");
		  ui_color(255,0,0,255);
...
static void paint_redraw(struct touch *tpd) {
    ...
    case ITEM_TOUCH_SENSITIVITY: paint_sensitivity_atp(tpd); break;
...
void add_event_atp(struct input_event *ev) {
...
void mmul_atp(float matrix_atp1[][3], float matrix_atp2[][3], float result[][3]) 
{
    int i, j, k;
    ...
    for (i = 0; i < 3; i++) 
    {
        for (j = 0; j < 3; j++) 
        {
            result[i][j] = 0;
            for (k = 0; k < 3; k++) 
                result[i][j] += (matrix_atp1[i][k] * matrix_atp2[k][j]);
        }
    }
}

void identity_atp(float src[][3]) 
{
    int i,j;
    for (i = 0; i < 3; i++) 
        for(j = 0; j < 3; j++) 
...
void duplicate_atp(float src[][3], float des[][3]) 
{
    int i, j;
    for (i = 0; i < 3; i++) 
        for (j = 0; j < 3; j++)
...
void vadd_atp(float vec1[3], float vec2[3]) 
{
    int i;
    for (i = 0; i < 3; i++) vec1[i] = vec1[i] + vec2[i];
}

void vmul_atp(float vec[3], float coef) 
{
    int i;
    for (i = 0; i < 3; i++) vec[i] *= coef;
}

void vrmul_atp(float src[3], float coef, float des[3]) 
{
    int i;
    for(i = 0; i < 3; i++) des[i] = src[i] * coef;
}

int inverse_mul(float src[][3], float des[][3])
{
    float dup[3][3], bak[3];
    int i, j;
    ...
    if (dup[0][0] == 0) 
    {
        vadd_atp(dup[0], dup[1]);
        vadd_atp(des[0], des[1]);
    } 
    else 
    {
        vadd_atp(dup[0], dup[2]);
        vadd_atp(des[0], des[2]);
    }
    
    vmul_atp(des[0], 1.0f / dup[0][0]);
    vmul_atp(dup[0], 1.0f / dup[0][0]);
    
    vrmul_atp(des[0], -dup[1][0], bak);
    vadd_atp(des[1], bak);
    vrmul_atp(dup[0], -dup[1][0], bak);
    vadd_atp(dup[1], bak);
    
    vrmul_atp(des[0], -dup[2][0], bak);
    vadd_atp(des[2], bak);
    vrmul_atp(dup[0], -dup[2][0], bak);
    vadd_atp(dup[2], bak);
    ...
    if (dup[1][1] == 0) 
    { 
        vadd_atp(dup[1], dup[2]);
        vadd_atp(des[1], des[2]); 
    }
    
    vmul_atp(des[1], 1.0f / dup[1][1]);
    vmul_atp(dup[1], 1.0f / dup[1][1]);
    
    vrmul_atp(des[1], -dup[0][1], bak);
    vadd_atp(des[0], bak);
    vrmul_atp(dup[1], -dup[0][1], bak);
    vadd_atp(dup[0], bak);
    
    vrmul_atp(des[1], -dup[2][1], bak);
    vadd_atp(des[2], bak);
    vrmul_atp(dup[1], -dup[2][1], bak);
    vadd_atp(dup[2], bak);
    
    if (dup[2][2] == 0) return FAIL;
    
    vmul_atp(des[2], 1.0f / dup[2][2]);
    vmul_atp(dup[2], 1.0f / dup[2][2]);
    
    vrmul_atp(des[2], -dup[0][2], bak);
    vadd_atp(des[0], bak);
    vrmul_atp(dup[2], -dup[0][2], bak);
    vadd_atp(dup[0], bak);
    
    vrmul_atp(des[2], -dup[1][2], bak);
    vadd_atp(des[1], bak);
    vrmul_atp(dup[2], -dup[1][2], bak);
    vadd_atp(dup[1], bak);
...
void calibrate_atp() 
{
    ...
    float ret_matrix_atp[3][3] = {{0, 0, 0}, {0, 0, 0}, {0, 0, 0}};
    float inv_matrix_atp[3][3] = {{0, 0, 0}, {0, 0, 0}, {0, 0, 0}};
    
    float tpd_matrix_atp[3][3] = {
        {tpd_point[0].x, tpd_point[0].y, 1},
        {tpd_point[1].x, tpd_point[1].y, 1},
        {tpd_point[2].x, tpd_point[2].y, 1}
    };
    
    float lcd_matrix_atp[3][3] = {
        {lcd_point[0].x, lcd_point[0].y,0},
        {lcd_point[1].x, lcd_point[1].y,0},
        {lcd_point[2].x, lcd_point[2].y,0}
    };
    LOGI(TAG "tpd_matrix_atp =");
    for (i = 0; i < 3; i++) 
        for (j = 0; j < 3; j++) 
            LOGI(TAG "%10.6f ", tpd_matrix_atp[i][j]);
    LOGI(TAG "\n");	

    LOGI(TAG "lcd_matrix_atp =");
    for (i = 0; i < 3; i++) 
        for (j = 0; j < 3; j++) 
            LOGI(TAG "%10.6f ", lcd_matrix_atp[i][j]);
    LOGI(TAG "\n");		
    
    inverse_mul(tpd_matrix_atp, inv_matrix_atp);
    mmul_atp(inv_matrix_atp, lcd_matrix_atp, ret_matrix_atp);
    for (i = 0; i < 2; i++) 
        for (j = 0; j < 3; j++) 
            calmat_atp[i][j] = (int)(ret_matrix_atp[j][i] * 4096);
    
    tmp_x = (int)((calmat_atp[0][0] * tpd_point[3].x + calmat_atp[0][1] * tpd_point[3].y + calmat_atp[0][2]) / (long)4096);
    tmp_y = (int)((calmat_atp[1][0] * tpd_point[3].x + calmat_atp[1][1] * tpd_point[3].y + calmat_atp[1][2]) / (long)4096);
    
    bias_atp[0] = lcd_point[3].x - tmp_x;
    bias_atp[1] = lcd_point[3].y - tmp_y;
    
    for (i = 0; i < 2; i++) 
        for (j = 0; j < 3; j++) 
            matrix_atp[i * 3 + j] = (int)calmat_atp[i][j];
            
    matrix_atp[6] = bias_atp[0];
    matrix_atp[7] = bias_atp[1];
...
int save_atp()
{
    ...
    des[0] = (int)((matrix_atp[0] * (long long)def[0] + matrix_atp[1] * (long long)def[3]) / 4096);
    des[1] = (int)((matrix_atp[0] * (long long)def[1] + matrix_atp[1] * (long long)def[4]) / 4096);
    des[2] = (int)((matrix_atp[0] * (long long)def[2] + matrix_atp[1] * (long long)def[5] + (long long)matrix_atp[2] * 4096) / 4096);
    des[3] = (int)((matrix_atp[3] * (long long)def[0] + matrix_atp[4] * (long long)def[3]) / 4096);
    des[4] = (int)((matrix_atp[3] * (long long)def[1] + matrix_atp[4] * (long long)def[4]) / 4096);
    des[5] = (int)((matrix_atp[3] * (long long)def[2] + matrix_atp[4] * (long long)def[5] + (long long)matrix_atp[5] * 4096) / 4096);
    des[6] = matrix_atp[6];
    des[7] = matrix_atp[7];
...
static void *touch_update_thread(void *priv) {
    ...
    while (1) {
		...
		case ITEM_TOUCH_CALIBRATION:
			    ...
			    case ITEM_TOUCH_MAIN:
					...
					{					   
					  if((x>0&&x<TOUCH_L1)&& (y>0&&y<TOUCH_L1))
					  {
						...
						tangle_atp_px1 = y;
						tangle_atp_l1 =1;
						LOGD(TAG "tangle_atp_px1=%d tangle_atp_py1=%d\n",tangle_atp_px1,tangle_atp_py1);
					  }
					  if(x> tpd->width-TOUCH_L1 && (y >0&&y<TOUCH_L1))
					  {
						...
						tangle_atp_py2 = y;
						tangle_atp_l2 =1;
						LOGD(TAG "tangle_atp_px2=%d tangle_atp_py2=%d\n",tangle_atp_px2,tangle_atp_py2);
					  ...
					  if(x> tpd->width-TOUCH_L1 && y >tpd->height-TOUCH_L1)
					  {
						...
						tangle_atp_py3 = y;
						tangle_atp_l3 =1;
						LOGD(TAG "tangle_atp_px3=%d tangle_atp_py3=%d\n",tangle_atp_px3,tangle_atp_py3);
					  ...
					  if(x<TOUCH_L1 && y >tpd->height-TOUCH_L1)
					  {
						...
						tangle_atp_py4 = y;
						tangle_atp_l4 =1;
					  	LOGD(TAG "tangle_atp_px4=%d tangle_atp_py4=%d\n",tangle_atp_px4,tangle_atp_py4);
					  }

					  	LOGD(TAG "draw circle length=%d\n",((tangle_atp_px2-tangle_atp_px1)+(tangle_atp_py3-tangle_atp_py2)+(tangle_atp_px3-tangle_atp_px4)+(tangle_atp_py4-tangle_atp_py1)));
						LOGD(TAG "pass circle length=%d\n",(2*(tpd->width-2*TOUCH_L1)+2*(tpd->height-2*TOUCH_L1)));
                       LOGD(TAG "tangle_atp_l1=%d\n",tangle_atp_l1);
						LOGD(TAG "tangle_atp_l2=%d\n",tangle_atp_l2);
						LOGD(TAG "tangle_atp_l3=%d\n",tangle_atp_l3);
						LOGD(TAG "tangle_atp_l4=%d\n",tangle_atp_l4);
					detect_flags = 1;
					i = 0;
					...
					if(detect_flags != 0)
					{
						...
						g_pass_case_atp1 =1;
						clear_touch_temp_data_atp();  
						//part(tpd,ITEM_TOUCH_LINEARITY);
						clear_detect_date();

                        if (get_is_ata()) g_detected_atp = 1;
					}
					i = 0;	
			}
					  
			if(1 == g_pass_case_atp1 && 0 == g_pass_case_atp2 && 0 == g_pass_case_atp3)
			{
				detect_flags = 1;
				i = 0;
				while( i < tpd->height/TOUCH_L1 && (g_pass_case_atp2 == 0) && (g_pass_case_atp3 == 0))
				{
			...
			if(1 == g_pass_case_atp1 && 1 == g_pass_case_atp2 && 1 ==g_pass_case_atp3)
			{
				...
				{					
                   g_detected_atp = 2;
                   LOGD(TAG "Set g_detected_atp = true for ATA.\n");
				}
				break;
			}
			if(1 == g_pass_case_atp1 && 1 == g_pass_case_atp2 && 1 ==g_pass_case_atp3 && 1 == g_pass_case_atp4)
			{
				if(y> 0 && y<TOUCH_L1)
					   {
						//tangle_atp_px1 = x;
...
/* Vanzo:songlixin on: Fri, 24 Jul 2015 18:21:55 +0800
 * any input let's know  detected
 */
                    if (get_is_ata())
                        g_detected_atp = 2;
// End of Vanzo:songlixin
...
void setup_cpv_atp(struct touch *tpd) {
    switch(tpd->scene) {
...
void setup_circles_atp(struct touch *tpd) {
    int i,j;
    int circles[][5] = {
...
int touch_entry_atp(struct ftm_param *param, void *priv) {
    bool exit = false;
    struct touch *tpd = (struct touch *)priv;
    ...
    if (type == '0') {
        tpd->menu.set_items(&(tpd->menu), rtype_touch_items_atp, 0);
    } else {
        tpd->menu.set_items(&(tpd->menu), ctype_touch_items_atp, 0);
    }
    
    pthread_create(&(tpd->update_thd), NULL, touch_update_thread, priv);
	clear_touch_result_atp();
    do {
        switch(tpd->scene) {
        ...
        default:
            ...
            pthread_mutex_lock(&enter_mutex);
	    if(tpd->scene != ITEM_TOUCH_CHOOSER || g_pass_case_atp1 != 0)
            {
            tpd->enter_scene = true;
            }
            pthread_mutex_unlock(&enter_mutex);
		...
		// For ATA test		
		if (get_is_ata() && g_detected_atp == 2) 
		{			
			tpd->mod->test_result = FTM_TEST_PASS;			
			tpd->exit_thd = true;		
		}
...
int atp_init(void)
{
    ...
    LOGD(TAG "%s\n", __FUNCTION__);
    //calibration_init_atp();
    mod = ftm_alloc(ITEM_ATP, sizeof(struct touch));
    ...
    ret = ftm_register(mod, touch_entry_atp, (void*)tpd);
    return 0;
}
