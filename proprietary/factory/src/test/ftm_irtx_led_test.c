...
/* Vanzo:songlixin on: Tue, 09 Jun 2015 20:36:11 +0800
 */
char irda_data_to_write[] = { 
0x00, 0x94, 0x00, 0x00, 0x94, 0x70, 0x01, 0x56,
0x00, 0xab, 0x00, 0x15, 0x00, 0x16, 0x00, 0x15,
0x00, 0x16, 0x00, 0x15, 0x00, 0x16, 0x00, 0x15,
0x00, 0x16, 0x00, 0x15, 0x00, 0x16, 0x00, 0x15,
0x00, 0x16, 0x00, 0x15, 0x00, 0x16, 0x00, 0x15,
0x00, 0x16, 0x00, 0x15, 0x00, 0x40, 0x00, 0x15,
0x00, 0x40, 0x00, 0x15, 0x00, 0x40, 0x00, 0x15,
0x00, 0x40, 0x00, 0x15, 0x00, 0x40, 0x00, 0x15,
0x00, 0x40, 0x00, 0x15, 0x00, 0x16, 0x00, 0x15,
0x00, 0x40, 0x00, 0x15, 0x00, 0x16, 0x00, 0x15,
0x00, 0x16, 0x00, 0x15, 0x00, 0x16, 0x00, 0x15,
0x00, 0x16, 0x00, 0x15, 0x00, 0x40, 0x00, 0x15,
0x00, 0x16, 0x00, 0x15, 0x00, 0x16, 0x00, 0x15,
...
0x00, 0x40, 0x00, 0x15, 0x00, 0x16, 0x00, 0x15,
0x00, 0x40, 0x00, 0x15, 0x00, 0x40, 0x00, 0x15,
0x00, 0x40, 0x00, 0x15, 0x06, 0x15, 0x01, 0x56,
0x00, 0x55, 0x00, 0x15, 0x0b, 0x61, 
};
pthread_t irtx_looper;
int irtx_loop_forever(void *priv)
{
    int fd = -1;
    while (1) {
        sleep(1);
        fd = open("/dev/remote_ctrl_dev", O_WRONLY);
        if (fd >= 0) {
            write(fd, irda_data_to_write, sizeof(irda_data_to_write));
        }
        close(fd);
    }
    pthread_exit(NULL);
...
int irtx_led_test_pre_set(void *priv)
{
    ...
    test_steps = 0;
    while (try_times-- > 0) {
/* Vanzo:songlixin on: Tue, 09 Jun 2015 21:03:39 +0800
...
/* Vanzo:songlixin on: Tue, 09 Jun 2015 21:11:24 +0800
            ret = ioctl(fd, IRTX_IOC_SET_IRTX_LED_OUT, &enable);
 */
            ret = write(fd, irda_data_to_write, sizeof(irda_data_to_write) );
// End of Vanzo: songlixin
            close(fd);

/* Vanzo:songlixin on: Wed, 10 Jun 2015 14:02:42 +0800
            if (ret) {
 */
            if (ret < 0) {
// End of Vanzo: songlixin
...
int irtx_led_entry(struct ftm_param *param, void *priv)
{
    ...
    if (FTM_MANUAL_ITEM == param->test_type) {
        ...
        if (test_steps == 2) {
            ...
            if (test_steps == 2) {
/* Vanzo:songlixin on: Thu, 11 Jun 2015 20:12:05 +0800
                auto_pass == true;
 */
                auto_pass = true;
// End of Vanzo: songlixin
...
/* Vanzo:songlixin on: Fri, 12 Jun 2015 11:01:26 +0800
 */
            pthread_create(&irtx_looper, NULL, irtx_loop_forever, NULL);
// End of Vanzo:songlixin
        } else {
            irtxledstu->mod->test_result = FTM_TEST_FAIL;
        }
