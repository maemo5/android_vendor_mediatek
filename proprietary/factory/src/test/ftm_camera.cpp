...
/* Vanzo:songlixin on: Tue, 19 May 2015 21:14:17 +0800
 */
#include "flash_drv.h"
...
/* Vanzo:songlixin on: Mon, 21 Sep 2015 21:00:54 +0800
 */
pthread_t strobe_looper;
static void *strobe_loop_forever(void *priv)
{
    FlashSimpleDrv*  pStrobe;
    FlashSimpleDrv2*  pStrobe2;

    pStrobe = FlashSimpleDrv::getInstance();
    pStrobe2 = FlashSimpleDrv2::getInstance();
...
/* Vanzo:songlixin on: Thu, 17 Apr 2014 15:13:27 +0800
 */
  {
      FILE *in;
      char buffer[256];
      int is_sensor_alive = 0;

      in = fopen("/proc/mtkdev", "r");
      if(in != NULL){
          while(fgets(buffer, sizeof(buffer), in)){
              if(srcDev == 2){ // sub sensor
                  if(buffer[0] == 'C' && buffer[1] == 'A' && buffer[2] == 'M' && buffer[3] == '2'){
                      FTM_CAMERA_DBG("CRC_accum Sub Camera strlen %d\n", strlen(buffer));
                      if(strlen(buffer) >= 8)
                          is_sensor_alive = 1;
                      break;
                  }
              }else{
                  if(buffer[0] == 'C' && buffer[1] == 'A' && buffer[2] == 'M' && buffer[3] == ':'){
                      FTM_CAMERA_DBG("CRC_accum Main Camera strlen %d\n", strlen(buffer));
                      if(strlen(buffer) >= 8)
                          is_sensor_alive = 1;
                      break;
                  }
...
{
/* Vanzo:songlixin on: Wed, 12 Mar 2014 21:29:26 +0800
 */
    if(srcDev == 1)  {
        pthread_create(&strobe_looper, NULL, strobe_loop_forever, NULL);
    }
// End of Vanzo:songlixin
