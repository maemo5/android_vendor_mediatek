...
public class FMTransmitterActivity extends Activity {
    ...
    // private static final String FMTRANSMITTERACTIVITY_FRAGMENT_TAG =
    // "FMTransmitterTag";

    // Seek parameters.
    private static final int SEEK_CHANNEL_COUNT = 1;
    private static final int SEEK_CHANNEL_DIRECTION = 0;
    private static final int SEEK_CHANNEL_GAP = FMTransmitterFeatureOption.IS_FM_50KHZ_SUPPORT ? 5 : 1;
    ...
    private static final int BASE_NUMBER = FMTransmitterFeatureOption.IS_FM_50KHZ_SUPPORT ? 100 : 10;
    ...
    private class FMTxAppBroadcastReceiver extends BroadcastReceiver {
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(FMTransmitterService.ACTION_STATE_CHANGED)) {
                mIsPlaying = intent.getBooleanExtra(FMTransmitterService.EXTRA_FMTX_ISPOWERUP,
                        false);
    ...
    private void uiComponentInit() {
        ...
        if (FMTransmitterFeatureOption.IS_FM_50KHZ_SUPPORT) {
    ...
    public void onStart() {
        ...
        if (null != cn) {
                            ...
                            if (!isSearching()) {
                                ...
                                try {
                                    if (!mService.isSearching() && sFragmentManager != null) {
                                        DialogFragment prefragdialog = (DialogFragment) sFragmentManager
                                ...
                                // Avoid show seek dialog when service do search which
                                // trigger by advance activity
                                if (sFragmentManager != null && mIsSeeking) {
            ...
            mIsServiceBinded = bindService(new Intent(FMTransmitterActivity.this,
                    FMTransmitterService.class), mServiceConnection, Context.BIND_AUTO_CREATE);
        }
        if (!mIsServiceBinded) {
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (RESULT_OK == resultCode) {
            if (REQUEST_CODE_ADVANCED == requestCode) {
                int iStation = data.getIntExtra(FMTransmitterAdvanced.ACTIVITY_RESULT,
                        mCurrentStation);
    ...
    private void enableRDS() {
        ...
        if (isRDSTxSupport()) {
            if (isRDSOn() || setRDSTxEnabled(true)) {
                if (isRDSOn() || setRDSTxEnabled(true)) {
                    if (!FMTransmitterFeatureOption.IS_BSP_PACKAGE) {
                        String defaultValue = "Mediatek";
                        String value = CustomProperties.getString(
                                CustomProperties.MODULE_FMTRANSMITTER, CustomProperties.RDS_VALUE,
                                defaultValue);
                        int rdsLength = (value.length() > RDS_LENGTH_MAX ? RDS_LENGTH_MAX : value
                                .length());
    ...
    public float[] searchChannelsForTx(float frequency, int direction, int number) {
        FMTxLogUtils.d(TAG, ">>> FMTransmitterActivity.searchChannelsForTx: " + frequency
                + direction + number);
        float[] ret = null;
        if (null == mService) {
            FMTxLogUtils.e(TAG, "Error: No service interface.");
        ...
        FMTxLogUtils.d(TAG,
                "<<< FMTransmitterActivity.searchChannelsForTx: " + Arrays.toString(ret));
        return ret;
    }
    ...
    private class PlayFMTxAsyncTask extends AsyncTask<Void, Void, Void> {
        ...
        protected void onPostExecute(Void result) {
            ...
            FMTxLogUtils.i(TAG, "[Performance test][FMTransmitter] Test FM Tx Power on end ["
                    + endPowerTime + "]");
            FMTxLogUtils.i(TAG, "[Performance test][FMTransmitter] Test FM Tx Power down end ["
                    + endPowerTime + "]");
        }
    ...
    private class SeekAsyncTask extends AsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... arg0) {
            ...
            float[] channels = searchChannelsForTx((float) iStation / BASE_NUMBER,
                    SEEK_CHANNEL_DIRECTION, SEEK_CHANNEL_COUNT);
            if (null == channels) {
                FMTxLogUtils.v(TAG, "Seek again from the lowest frequency");
                channels = searchChannelsForTx((float) FMTransmitterStation.LOWEST_STATION
        ...
        protected void onPostExecute(Void result) {
            ...
            FMTxLogUtils.i(TAG,
                    "[Performance test][FMTransmitter] Test FM Tx total seek time end ["
                            + endSeekTime + "]");
            mIsSeeking = false;
        }
