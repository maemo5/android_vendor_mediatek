...
public class OriginatingOne2OneStandAloneChatSession extends OneOneChatSession {	
	 ...
	 */
	public OriginatingOne2OneStandAloneChatSession(ImsService parent, String contact, InstantMessage msg, String largeMessage) {
		...
		if(RcsSettings.getInstance().isCPMSupported()) {
			if (logger.isActivated()) {
	    		logger.info("CPMS OriginatingOne2OneStandAloneChatSession old call id: " + getDialogPath().getCallId());
	    	}
			if(RichMessagingHistory.getInstance().getCoversationID(newContact,1).equals("")) {
				// Create a text message
		        InstantMessage conversationMsg = ChatUtils.createTextMessage(newContact, "system",
				...
				if (logger.isActivated()) {
		    		logger.info("CPMS OriginatingOne2OneStandAloneChatSession ConversationId: " + ConversationId);
		    	}
				...
				if (logger.isActivated()) {
		    		logger.info("CPMS OriginatingOne2OneStandAloneChatSession ConversationId1: " + getConversationID());
		    	}
			}
		}
	}
        ...
        logger.debug("createSipInvite()");
            ...
            if(isBurnMessage()){
				...
				try {
				    ...
				    String headerString = invite.getHeader(SipUtils.HEADER_ACCEPT_CONTACT).toString();
					...
					acceptContactHeader = SipUtils.HEADER_FACTORY.createHeader(SipUtils.HEADER_ACCEPT_CONTACT, headerString);
	           		invite.getStackMessage().setHeader(acceptContactHeader);   
    ...
    public void msrpDataTransfered(String msgId) {
		...
		if (msgId != null) {
	        ...
	        for(int i=0; i < getListeners().size(); i++) {
                ((ChatSessionListener)getListeners().get(i)).handleMessageDeliveryStatus(contact,msgId, ImdnDocument.DELIVERY_STATUS_SENT);
	        }
        } 
