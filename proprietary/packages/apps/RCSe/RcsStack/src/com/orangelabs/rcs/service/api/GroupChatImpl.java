...
public class GroupChatImpl extends IGroupChat.Stub implements ChatSessionListener {
    ...
    public void modifyMyNickName(final String newNickname) {
        ...
        //RichMessagingHistory.getInstance().updateGroupNickName(getChatId(), newNickname);
		/*if (logger.isActivated()) {
				logger.info("GCM modifyMyNickName Contact: " + RcsSettings.getInstance().getUserProfileImsUserName() + "nickname: " + newNickname);
		}
		RichMessagingHistory.getInstance().updateGroupMemberName(session.getContributionID(),RcsSettings.getInstance().getUserProfileImsUserName() ,newNickname);*/
        final String contact = RcsSettings.getInstance().getUserProfileImsUserName();
        Thread t = new Thread() {
            public void run() {
    ...
    public void handleSessionStarted() {
    	synchronized(lock) {
	    	...
	    	if(!(RichMessagingHistory.getInstance().isGroupMemberExist(getChatId(),RcsSettings.getInstance().getUserProfileImsUserName()))){//TODO why not get from IMS profile instead of db
	    		RichMessagingHistory.getInstance().addGroupMember(session.getContributionID(),RcsSettings.getInstance().getJoynUserAlias(),RcsSettings.getInstance().getUserProfileImsUserName(),null);
	    	}
    	...
    	// Generate a message Id
        if (logger.isActivated()) {
		logger.info("GCM Session starte multi message ID:" + multiMessage.getMessageId());
	}
        final String msgId = ChatUtils.generateMessageId();
        
        if(LauncherUtils.supportOP01()) {
	        // Update rich messaging history
			InstantMessage msg = new InstantMessage(msgId, getRemoteContact(), multiMessage.getTextMessage(), session.getImdnManager().isImdnActivated() ,null);
			RichMessagingHistory.getInstance().addGroupChatMessage(session.getContributionID(), msg,
					ChatLog.Message.Direction.OUTGOING);
        }
    	...
    	Thread t = new Thread() {
            public void run() {
	                session.sendTextMessage(multiMessage.getMessageId(), multiMessage.getTextMessage());
	                multiMessage = null;
            }
        };
        t.start();
