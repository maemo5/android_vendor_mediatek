...
public class ChatImpl extends IChat.Stub implements ChatSessionListener {
    ...
    private final static String REQUEST_MIME_TYPE = "message/CPIM";
    ...
    public String sendMessageByPagerMode(String message, boolean isBurnMessage,
            boolean isPublicMessage, boolean isMultiMessage,boolean isPayEmoticon, List<String>  participants) {
        return sendMessageByPagerMode( message,  isBurnMessage, isPublicMessage,  isMultiMessage, isPayEmoticon,null,  participants);
        
    }

    
    public String sendMessageByPagerMode(String message, boolean isBurnMessage,boolean isPublicMessage, boolean isMultiMessage,boolean isPayEmoticon,String resendMsgId, List<String>  participants) {
        if (logger.isActivated()) {
            logger.debug("sendMessageByPagerMode Burn:" + isBurnMessage + "Public:" + isPublicMessage + " MultiMessage "
        ...
        // Create a text message
        InstantMessage msgg = ChatUtils.createTextMessage(contact, message, Core.getInstance().getImService().getImdnManager().isImdnActivated());
        if (resendMsgId != null) {
            msgg.setMessageId(resendMsgId);
            msgg.setBurnMessage(RichMessagingHistory.getInstance().isBurnMessage(resendMsgId));
            msgg.setBurnMessage(RichMessagingHistory.getInstance().isPublicMessage(resendMsgId));
        }
        ...
        if (isPayEmoticon == true)
            ...
            if (RcsSettings.getInstance().isImReportsActivated()) {
            	...
            	try{
	            	if (logger.isActivated()) {
	                    logger.debug("Send text237 message:" + Base64.encodeToString(message.getBytes(), Base64.NO_WRAP));
	                }
	            	if (logger.isActivated()) {
                ...
                content = ChatUtils.buildCpimMessageWithImdn(from, to, msgg.getMessageId(), StringUtils.encodeUTF8(message),
            	...
            	try{
            	if (logger.isActivated()) {
                    logger.debug("Send text236 message:" + Base64.encodeToString(message.getBytes(), Base64.NO_WRAP));
                }
            }catch(Exception e){
            }
            ...
            if (isBurnMessage) {
                ...
                RichMessagingHistory.getInstance().addBurnChatMessage(msgg, ChatLog.Message.Direction.OUTGOING);
            } else if(isPublicMessage) {
                msgg.setPublicMessage(isPublicMessage);
                RichMessagingHistory.getInstance().addPublicChatMessage(msgg, ChatLog.Message.Direction.OUTGOING);
            }else{
                RichMessagingHistory.getInstance().addChatMessage(msgg, ChatLog.Message.Direction.OUTGOING);
            }
            }
        ...
        try {
            ...
            if (isPublicMessage) {
                ...
                //donot make
                 dialogPath = new SipDialogPath(imsService.getImsModule().getSipManager().getSipStack(),
            ...
            // Create a dialog path
             dialogPath = new SipDialogPath(imsService.getImsModule().getSipManager().getSipStack(), imsService
                    .getImsModule().getSipManager().getSipStack().generateCallId(), 1,
                    PhoneUtils.formatNumberToSipUri(contact), ImsModule.IMS_USER_PROFILE.getPublicUri(),
                    PhoneUtils.formatNumberToSipUri(contact), imsService.getImsModule().getSipManager().getSipStack()
                            .getServiceRoutePath());
            }

            SipRequest request = SipMessageFactory.createMessageCPM(dialogPath, featureTags, mime, content);

            // Allow header
            SipUtils.buildAllowHeader(request.getStackMessage());

            // Set contribution Id
            String contributionId = ContributionIdGenerator.getContributionId(dialogPath.getCallId());
            request.addHeader(ChatUtils.HEADER_CONTRIBUTION_ID, contributionId);

            // set Conversation Id
            String conversationId = (RichMessagingHistory.getInstance().getCoversationID(this.getRemoteContact(), 1));
            ...
            if (isPublicMessage) {
                String headerString = ((SIPHeader)request.getHeader(SipUtils.HEADER_ACCEPT_CONTACT)).getHeaderValue();
                headerString += ";" + FeatureTags.FEATURE_CMCC_PUBLIC_ACCOUNT;
                Header acceptContactHeader;
                acceptContactHeader = SipUtils.HEADER_FACTORY.createHeader(SipUtils.HEADER_ACCEPT_CONTACT, headerString);
            ...
            if (isBurnMessage) {
                ...
                try {
                    ...
                    String headerString = request.getHeader(SipUtils.HEADER_ACCEPT_CONTACT).toString();
                    logger.debug("old value : " + headerString);
                    headerString += ";" + FeatureTags.FEATURE_RCSE_CPM_BURN_AFTER_READING_FEATURE_TAG;
                    logger.debug("new value : " + headerString);
                    Header acceptContactHeader;
                    acceptContactHeader = SipUtils.HEADER_FACTORY.createHeader(SipUtils.HEADER_ACCEPT_CONTACT, headerString);
                    request.getStackMessage().setHeader(acceptContactHeader);
                ...
                public void run() {
                    ...
                    try {
                        ...
                        if ((ctx.getStatusCode() == 200) || (ctx.getStatusCode() == 202)) {
                            ...
                            handleMessageDeliveryStatus(number, msg.getMessageId(),ImdnDocument.DELIVERY_STATUS_SENT);
                            ...
                            handleMessageDeliveryStatus(number, msg.getMessageId(), ImdnDocument.DELIVERY_STATUS_ERROR);
                        }
                    } catch (Exception e) {
                    	 if (logger.isActivated()) {
            ...
            handleMessageDeliveryStatus(msgg.getRemote(), msgg.getMessageId(), ImdnDocument.DELIVERY_STATUS_ERROR);
        }
	...
	public String sendOnetoMultiMessage(String message,  List<String> participants) {
	   ...
	   /* this should be used by application */
	    ...
	    public int reSendMultiMessageByPagerMode(String messageID) {
	         ...
	         sendMultiMessageByPagerMode(RichMessagingHistory.getInstance().getMessageText(messageID),messageID, RichMessagingHistory.getInstance().getMultiMessageParticipants(messageID));
    ...
    public String sendMultiMessageByPagerMode(String message,String msgId, List<String> participants) {
        if (logger.isActivated()) {
        ...
        // Create a text message
        InstantMessage msgg = ChatUtils.createTextMessage(contact, message, Core.getInstance().getImService()
        ...
        if(msgId!=null) {
            ...
            msgg.setBurnMessage( RichMessagingHistory.getInstance().isPublicMessage(msgId) );
        ...
        content = Multipart.BOUNDARY_DELIMITER + BOUNDARY_TAG + SipUtils.CRLF
                ...
                + "Content-Type:"+ REQUEST_MIME_TYPE + SipUtils.CRLF
                + SipUtils.CRLF 
        ...
        if(RcsSettings.getInstance().isBase64EncodingSupported()){   
             ...
             int contentLength = m.getBytes().length;;
              content = content  + CpimMessage.HEADER_CONTENT_TYPE + ": " + "text/plain" + ";charset=utf-8" + SipUtils.CRLF
            ...
            content = content    + CpimMessage.HEADER_CONTENT_TYPE + ": " + "text/plain" + ";charset=utf-8" + SipUtils.CRLF
                + CpimMessage.HEADER_CONTENT_LENGTH + ": " + message.getBytes().length + SipUtils.CRLF
                + message + SipUtils.CRLF
                + Multipart.BOUNDARY_DELIMITER + BOUNDARY_TAG + Multipart.BOUNDARY_DELIMITER;
        }

        /*   RichMessagingHistory.getInstance().addMultiMessageChat(null, msgg.getTextMessage(), msgg.getMessageId(),
        participants, ChatLog.Message.Status.Content.SENDING, ChatLog.Message.Direction.OUTGOING);
*/
...
RichMessagingHistory.getInstance().addChatMessage(msgg, ChatLog.Message.Direction.OUTGOING);
        ...
        try {
            ...
            SipDialogPath dialogPath = new SipDialogPath(imsService.getImsModule().getSipManager().getSipStack(),
                    imsService.getImsModule().getSipManager().getSipStack().generateCallId(), 
                    1,
            ...
            SipRequest request = SipMessageFactory.createMessageMultiCPM(dialogPath, featureTags, mime, content);
            ...
            Thread t = new Thread() {
                public void run() {
                    ...
                    if ((ctx.getStatusCode() == 200) || (ctx.getStatusCode() == 202)) {
                        ...
                        if (logger.isActivated()) {
                            logger.info("20x OK response received");
                            handleMessageDeliveryStatus(number, msg.getMessageId(),
                                    ImdnDocument.DELIVERY_STATUS_SENT);
                        ...
                        if (logger.isActivated()) {
                            logger.info("failed: " + ctx.getStatusCode() + " response received");
                            handleMessageDeliveryStatus(number, msg.getMessageId(),
                                    ImdnDocument.DELIVERY_STATUS_FORBIDDEN);
                        }
                    } else {
                        // Error responses
                        if (logger.isActivated()) {
...
/**
   ...
   public int resendMessage(String msgId) {
     ...
     // Create a text message
        ...
        if(msg.getTextMessage().length() >1300){
            msg.setMessageId(msgId);
   ...
   public boolean isPublicAccountMessage(String msgId) {
    ...
    public String sendMessageByLargeMode(InstantMessage msg,boolean isPending,String[] extraParams) {
        if (logger.isActivated()) {
            logger.debug("LMM sendMessageByLargeMode isPending:" + isPending + " Text:" + msg.getTextMessage() + " msgId:" + msg.getMessageId());
        }
        synchronized(lock) {
                ...
                if(!isPending){
                    if(session != null){
                        ...
                        if (!RichMessagingHistory.getInstance().isOne2OneMessageExists(msg.getMessageId())) {
                        	...
                        	String message1 = RichMessagingHistory.getInstance().getMessageText(msg.getMessageId());
                            if (logger.isActivated()) {
                        ...
                        if(isBurnMessage){
                            msg.setBurnMessage(isBurnMessage);
                                RichMessagingHistory.getInstance().addBurnChatMessage(msg,
                        ...
                        if (logger.isActivated()) {
                            logger.debug("LMM Add into pending list text: " + message + " MsgId:" + msg.getMessageId());
                        }
                        if(RichMessagingHistory.getInstance().isOne2OneMessageExists(msg.getMessageId())){
                        	if (logger.isActivated()) {
                        ...
                        String message2 = RichMessagingHistory.getInstance().getMessageText(msg.getMessageId());
                        if (logger.isActivated()) {
                        ...
                        pendingMessageId = msg.getMessageId();
                         ...
                         if(isBurnMessage){
                            ...
                            if(isPublicAccountMessage(msg.getMessageId()) ||  msg.isPublicMessage()){
                            	session = (OneOneChatSession)Core.getInstance().getImService().initiateOne2OnePublicChatSession(contact, null, message);
                            	 session.setPublicAccountMessage(true);
                            } else {
                            	 // Create a text message
                                InstantMessage tempMsg = ChatUtils.createTextMessage(this.contact, message,
                                        Core.getInstance().getImService().getImdnManager().isImdnActivated());
                                tempMsg.setMessageId(msg.getMessageId());
                            	session = (OneOneChatSession)Core.getInstance().getImService().initiateOne2OneStandAloneChatSession(contact, tempMsg, message);
                            }
                           //set burn message
                           ...
                           RichMessagingHistory.getInstance().addBurnChatMessage(msg, ChatLog.Message.Direction.OUTGOING);
                            }
                         }                   
                         else{
                             if (logger.isActivated()) {
                    ...
                    if (logger.isActivated()) {
                        logger.debug("LMM  initiate a new session to send the pending message text:" + message);
                    }
                    pendingMessage = message;
                 // Initiate a new session
                    if(isPublicAccountMessage(msg.getMessageId()) || msg.isPublicMessage()){
    ...
    public void sendDisplayedDeliveryReport(final String msgId) {
        try {
            if (logger.isActivated()) {
                logger.info("LMM Set displayed delivery report for " + msgId + "contact: " + session.getRemoteContact());
            }

            // Send delivery status
            if ((session != null) &&
                    (session.getDialogPath() != null) &&
                        (session.getDialogPath().isSessionEstablished()) && session.getMsrpMgr() != null && session.getMsrpMgr().getMsrpSession() != null && !session.isLargeMessageMode()) { 
                // Send via MSRP
                Thread t = new Thread() {
                    public void run() {
                ...
                if (logger.isActivated()) {
                    logger.info("LMM Set displayed delivery report for1 " + "newContact: " + newContact + "subStr: " + subStr);
                }
    ...
    public void sendBurnDeliveryReport(final String msgId) {
        try {
            ...
            if ((session != null) &&
                ...
                Thread t = new Thread() {
                    public void run() {
                        session.sendMsrpMessageBurnStatus(session.getRemoteContact(), msgId, ImdnDocument.BURN_STATUS_BURNED);
                    }
                };
                t.start();
            } else {
    ...
    public void handleSessionStarted() {
        ...
        if (logger.isActivated()) {
            logger.info("LMM handleSessionStarted:" + pendingMessage + "MsgId:" + pendingMessageId);
        }
        ...
        if(!mPendingMessage.isEmpty()){
            AsyncTask.execute(new Runnable() {
                ...
                public void run() {
                    ...
                    if (logger.isActivated()) {
                        logger.info("LMM handleSessionAborted text:" + imMessage.getTextMessage() + "MsgId:" + imMessage.getMessageId());
                    }
    ...
    public void handleSessionTerminatedByRemote() {
        synchronized(lock) {
            ...
            String number = PhoneUtils.extractNumberFromUri(remoteContact);
            if (logger.isActivated()) {
                logger.info("LMM handleSessionTerminatedByRemote: " + remoteContact + "Number: " + number);
            }
        ...
        if(!mPendingMessage.isEmpty()){
            AsyncTask.execute(new Runnable() {
                ...
                public void run() {
                    InstantMessage imMessage = mPendingMessage.get(0);
                    if (logger.isActivated()) {
                        logger.info("LMM handleSessionTerminatedByRemote text:" + imMessage.getTextMessage() + "MsgId:" + imMessage.getMessageId());
                    }
                    mPendingMessage.remove(imMessage);
                    pendingMessageId = imMessage.getMessageId();
    ...
    public void handleReceiveMessage(InstantMessage message) {
        synchronized(lock) {
            if (logger.isActivated()) {
                logger.info("LMM handleReceiveMessage New IM received: " + message.getTextMessage());
            }
            ...
            if(message.isPublicMessage()){
            	msgApi = new ChatMessage(message.getMessageId(),
                        message.getRemote(),
        ...
        if(!mPendingMessage.isEmpty()){
            AsyncTask.execute(new Runnable() {
                ...
                public void run() {
                    InstantMessage imMessage = mPendingMessage.get(0);
                    if (logger.isActivated()) {
                        logger.info("LMM handleImError text:" + imMessage.getTextMessage() + "MsgId:" + imMessage.getMessageId());
                    }
                    mPendingMessage.remove(imMessage);
                    pendingMessageId = imMessage.getMessageId();
                    sendMessageByLargeMode(imMessage, true, null);
