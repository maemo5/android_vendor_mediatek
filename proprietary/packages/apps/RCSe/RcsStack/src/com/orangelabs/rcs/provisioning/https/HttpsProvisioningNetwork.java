...
public class HttpsProvisioningNetwork implements NameResolver {
    ...
    public HttpsProvisioningNetwork(
            HttpsProvisioningManager httpsProvisioningManager) {
    ...
    public void acquireNetwork(long subId) throws NetworkException {
        ...
        Log.d("HttpsProvisioningNetwork", "NetworkManager:1111");
        synchronized (this) {
            mRequestCount += 1;
            if (mNetwork != null) {
                // Already available
                Log.d("HttpsProvisioningNetwork", "HttpsNetworkManager: already available");
                return;
            }
            Log.d("HttpsProvisioningNetwork", "NetworkManager: start new network request");
            ...
            while (waitTime > 0) {
                try {
                    ...
                    Log.w("HttpsProvisioningNetwork", "NetworkManager: acquire network wait interrupted");
                }
                if (mNetwork != null) {
                    // Success
                    return;
            ...
            Log.d("HttpsProvisioningNetwork", "NetworkManager: timed out");
    ...
    public void releaseNetwork() {
        synchronized (this) {
            if (mRequestCount > 0) {
                mRequestCount -= 1;
                Log.d("HttpsProvisioningNetwork", "NetworkManager: release, count=" + mRequestCount);
                if (mRequestCount < 1) {
                    releaseRequest(mNetworkCallback);
                    resetLocked();
                }
    ...
    private void newRequest(long subId) {
        ...
        mNetworkCallback = new ConnectivityManager.NetworkCallback() {
            ...
            public void onAvailable(Network network) {
                ...
                Log.d("HttpsProvisioningNetwork", "NetworkCallbackListener.onAvailable: network=" + network);
                synchronized (HttpsProvisioningNetwork.this) {
                    mNetwork = network;
                    Log.d("HttpsProvisioningNetwork", "NetworkCallbackListener.onAvailable: mNetwork=" + mNetwork);
                    Message msg = mHandler.obtainMessage(EVENT_ON_AVAILABLE, (Object) network);
                  //  mManager.connectionEvent(ConnectivityManager.CONNECTIVITY_ACTION);
                  //  mManager.needUpdateConfig();
                    Intent intent1 = new Intent(HttpsProvisioningManager.ACTION_NEED_UPDATE_CONFIG);
                    AndroidFactory.getApplicationContext().sendBroadcast(intent1);
            ...
            public void onLost(Network network) {
                ...
                Log.d("HttpsProvisioningNetwork", "NetworkCallbackListener.onLost: network=" + network);
                synchronized (HttpsProvisioningNetwork.this) {
                    releaseRequest(this);
            ...
            public void onUnavailable() {
                super.onUnavailable();
                Log.d("HttpsProvisioningNetwork", "NetworkCallbackListener.onUnavailable");
                synchronized (HttpsProvisioningNetwork.this) {
                    releaseRequest(this);
                    if (mNetworkCallback == this) {
                        resetLocked();
        ...
        Log.d("HttpsProvisioningNetwork", "newRequest, subId = " + subId);
    ...
    public ConnectivityManager getConnectivityManager() {
        if (logger.isActivated())
            logger.debug("On Seperate connection interface op01");
