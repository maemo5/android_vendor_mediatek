...
public class InstantMessagingService extends ImsService {
	...
	public void receiveGroupFileTransferInvitation(SipRequest invite) {
		...
		if(groupSession == null){
			groupSession = groupChatSession;
			if (logger.isActivated()) {
		    	logger.info("FTS Receive a file transfer session use default group");
		    }
		}
    ...
    public void receivePagerModeMessage(SipRequest request)
 {
    ...
    public void receiveGroupListNotification(SipRequest notify) {
        ...
        //get group list from notify
     // Parse XML part
        byte[] content = notify.getContentBytes();
        if (content != null) {
            try {
                InputSource pidfInput = new InputSource(new ByteArrayInputStream(content));
                GroupListParser listParser = new GroupListParser(pidfInput);
                GroupListDocument groupList = listParser.getGroupList();

                if (groupList != null) {
                    List<String> groupIds = groupList.getGroupIds();
                    if (logger.isActivated()) {
                        logger.debug("receiveGroupListNotification groupIds: " + groupIds);
                    }
        //update database with group list
                    //RichMessagingHistory.getInstance().updateEachGroupChatStatus(groupIds);
                }
            } catch(Exception e) {
                if (logger.isActivated()) {
