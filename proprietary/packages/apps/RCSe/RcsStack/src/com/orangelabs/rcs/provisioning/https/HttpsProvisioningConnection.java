...
public class HttpsProvisioningConnection {
    ...
    /**
     * Separete Network state listener
     */
    private BroadcastReceiver seperateNetworkStateListener = null;
     ...
     * Register the broadcast receiver for network state
     */
    protected void registerSeperateNetworkStateListener() {
        // Check if network state listener is already registered
        if (seperateNetworkStateListener != null) {
            if (logger.isActivated()) {
        ...
        // Instantiate the network state listener
        seperateNetworkStateListener = new BroadcastReceiver() {
            @Override
            public void onReceive(Context context, final Intent intent) {
                Thread t = new Thread() {
                    public void run() {
                        if (logger.isActivated()) {
                            logger.debug("Separete Network state listener - Received broadcast: "
                                    + intent.toString());
                        }
        ...
        // Register network state listener
        IntentFilter intentFilter = new IntentFilter();
        intentFilter.addAction(HttpsProvisioningManager.ACTION_NEED_UPDATE_CONFIG);
        manager.getContext().registerReceiver(seperateNetworkStateListener, intentFilter);
    }

    /**
     * Unregister the broadcast receiver for network state
     */
    protected void unregisterSepareteNetworkStateListener() {
        if (seperateNetworkStateListener != null) {
            if (logger.isActivated()) {
            ...
            try {
                manager.getContext().unregisterReceiver(seperateNetworkStateListener);
            } catch (IllegalArgumentException e) {
                // Nothing to do
            }
            seperateNetworkStateListener = null;
        }
    }
}
