...
public class RichMessagingHistory {
	 ...
	 * 
	 */
	public void addGroupMember(String chatId, String memberName, String memberNumber,String eType) {
		if (logger.isActivated()){
			logger.debug("Add group member: chatID=" + chatId + "  and memeber : " + memberName + " and memberNumber : " + memberNumber);
		}
		ContentValues values = new ContentValues();
	...
	public void removeGroupMember(String chatId,String memberNumber) {
		...
		if (logger.isActivated()) {
					logger.debug("removeGroupMember() : for chatId = "+ chatId + "; and memberNumber : "+memberNumber);
				}
        	// Delete entries
    		int deletedRows = cr.delete(grpMemberDatabaseUri, 
	...
	public void updateGroupMemberEtype(String chatId,String memberNumber, String eType){
		if (logger.isActivated()) {
			logger.debug("updateGroupMemberEtype for  chat id : " + chatId + "; member_number : "+ memberNumber + " ; etype : " + eType);
		}
		ContentValues values = new ContentValues();
		values.put(GroupMemberData.KEY_CONTACT_TYPE, eType);
	...
	public void updateGroupMemberName(String chatId,String memberNumber, String memberName){
		if (logger.isActivated()) {
			logger.debug("updateGroupMemberName for  chat id : " + chatId + "; member_number : "+ memberNumber + " ; memberName : " + memberName);
		}
		ContentValues values = new ContentValues();
		values.put(GroupMemberData.KEY_MEMBER_NAME, memberName);
		cr.update(grpMemberDatabaseUri, 
	...
	public boolean isGroupMemberExist(String chatId, String contact) {
    	...
    	Cursor cursor = cr.query(grpMemberDatabaseUri, 
    			...
    			"(" + GroupMemberData.KEY_CHAT_ID + "='" 
    			+ chatId + "') AND (" + GroupMemberData.KEY_CONTACT_NUMBER + "=" + contact + ")",
				null, 
    			null);
    	if(cursor==null){
    		return false;
	...
	private void addChatMessage(InstantMessage msg, int type, int direction) {
		String contact = null;
		if (logger.isActivated()){
			logger.debug("Add chat message: contact= " +  msg.getRemote());
		}
		...
		if (msg instanceof GeolocMessage) {
			...
			if ((msg.isPublicMessage()) && (direction == ChatLog.Message.Direction.INCOMING)) 
				values.put(MessageData.KEY_CONTENT_TYPE, "application/xml"); //TODO Define a place to put the value , get from xml , not hardcode
			else
	...
	public void addFileTransfer(String contact, String fileTransferId, int direction, MmContent content) {
		/*if(contact.contains("")){//multimessage
	 ...
	 * @param content File content 
	 */
	public void addBurnFileTransfer(String contact, String fileTransferId, int direction, MmContent content) {
		contact = PhoneUtils.extractNumberFromUri(contact);
		if (logger.isActivated()){
			logger.debug(new StringBuilder("Add burn file transfer entry: fileTransferId=").append(fileTransferId).append(", contact=")
					.append(contact).append(", filename=").append(content.getName()).append(", size=").append(content.getSize())
					.append(", MIME=").append(content.getEncoding()).toString());
		}
		ContentValues values = new ContentValues();
	...
	public void updateFileTransferDuration(String fileTransferId, int duration) {
		if (logger.isActivated()) {
			logger.debug("updateFileTransferStatus (duration=" + duration + ") (fileTransferId=" + fileTransferId + ")");
		}
		ContentValues values = new ContentValues();
	...
	public void updateFileTransferId(String fileTransferId, String newFileTransferId) {
        if (logger.isActivated()) {
            logger.debug("updateFileTransferId (New=" + newFileTransferId + ") (Old=" + fileTransferId + ")");
        }
        ContentValues values = new ContentValues();
  ...
  //type 1: o2o and type =2 grphcat
    public void UpdateCoversationID(String chatID,String conversationID, int type){
    	...
    	if(type ==1){
    		...
    		cr.update(msgDatabaseUri, values, MessageData.KEY_CHAT_ID + " = '" + chatID + "' AND "+MessageData.KEY_TYPE+ " = " +Type.SYSTEM, null);
    	}
    	else if(type == 2){
    		//grp chat
    		ContentValues values = new ContentValues();
     ...
     * (non-Javadoc)
     * 
     * @see com.orangelabs.rcs.provider.messaging.IMultiFileTransferLog#addOutgoingMultiFileTransfer(java.lang.String, java.lang.String,
     * com.orangelabs.rcs.core.content.MmContent, com.orangelabs.rcs.core.content.MmContent)
     */
    
    public void addOutgoingMultiFileTransfer(String fileTransferId, MmContent content, MmContent thumbnail,List <String>  participants) {
        if (logger.isActivated()) {
            logger.debug("addOutgoingMultiFileTransfer: fileTransferId=" + fileTransferId + "filename="
                    + content.getName() + ", size=" + content.getSize() + ", MIME=" + content.getEncoding());
        }
        ContentValues values = new ContentValues();
        
       
        values.put(MultiFileTransferData.KEY_PARTICIPANTS_LIST, RichMessagingHistory.getParticipants(participants));
    ...
    public boolean isMultiParticipantMessage(String chatID){
		...
		List<String> result = new ArrayList<String>();
		try {
	     	Cursor cursor = cr.query(multiParticipantDatabaseUri, 
	    			new String[] {
	     			MultiMessageData.KEY_PARTICIPANTS
	    			},
	    			 "(" + MultiMessageData.KEY_CHAT_ID+ "='" + chatID + "')",
	    			null, 
	    			MultiMessageData.KEY_TIMESTAMP + " DESC");
    ...
    public void addMultiMessageChat(String chatId,String message,String messageId,List<String> Participants, int status, int direction ){
    	if (logger.isActivated()){
    ...
    public void updateMultiMessageChatStatus(String chatId,int status){
		...
		ContentValues values = new ContentValues();
		values.put(MultiMessageData.KEY_STATUS, status);
		cr.update(multiParticipantDatabaseUri, 
				values, 
				MultiMessageData.KEY_CHAT_ID + " = '" + chatId + "'", 
				null);
    }
