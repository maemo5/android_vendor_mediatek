...
public class SipInterface implements SipListener {
   ...
   // private boolean sipTraceEnabled = RcsSettings.getInstance().isSipTraceActivated();
     ...
     * @throws RSTException 
     */
    public SipTransactionContext sendSipMessageAndWait(SipMessage message) throws SipException, RSTException {
        try {
            ...
            if (e.getCause() != null && e.getCause().toString().contains("IOException")){  
                if (logger.isActivated()) {
                    logger.error("socket closed ", e);
                }
              //this means IOhandler couldn't handle it so reregister.
                throw new RSTException("Can't send SIP message");
