...
public static ArrayList<PauseResumeFileObject> mFileTransferPauseResumeData = new ArrayList<PauseResumeFileObject>();

	/**
	 * List of spam sessions
	 */
	private static ArrayList<OriginatingFileSpamSession> spamSessions = new ArrayList<OriginatingFileSpamSession>();
	...
	protected static void addFileTransferSession(FileTransferImpl session) {
		if (logger.isActivated()) {
			logger.debug("FTS Add a file transfer session in the list (size=" + ftSessions.size() + ")" + " Id: " + session.getTransferId());
		}
	...
	protected static void removeFileTransferSession(String sessionId) {
		if (logger.isActivated()) {
			logger.debug("FTS Remove a file transfer session from the list (size=" + ftSessions.size() + ")" + " Id: " + sessionId);
		}
    ...
    public void receiveHttpFileTransferInvitation(String remote,FileSharingSession session, boolean isGroup) {
        if (logger.isActivated()) {
            logger.info("FTS Receive httpfile transfer invitation from " + remote + " isGroup " + isGroup);
        }
        String chatId = null;
		...
		RichMessagingHistory.getInstance().updateFileIcon( session.getSessionID(), session.getThumbUrl());
        ...
        if(RcsSettings.getInstance().isCPMSupported()) {
        	if(!(RichMessagingHistory.getInstance().getCoversationID(session.getRemoteContact(), 1).equals(session.getConversationID()))){
        		if(session.getConversationID() != null) {
	        		if (logger.isActivated()) {
	        	       logger.info("FTS receiveHttpFileTransferInvitation  OldId: "  + RichMessagingHistory.getInstance().getCoversationID(session.getRemoteContact(),1) + " NewId: " + session.getConversationID());
	        	    }
	                RichMessagingHistory.getInstance().UpdateCoversationID(session.getRemoteContact(),session.getConversationID(), 1);
        		} else {
        			if (logger.isActivated()) {
 	        	       logger.info("FTS receiveHttpFileTransferInvitation  conversation id is null");
 	        	    }
        		}
        	}
        }
        ...
        String chatSessionId =  null;
          if (session instanceof HttpFileTransferSession) {
            ...
            chatSessionId = ((TerminatingGroupFileSharingSession)session).getGroupChatSession().getSessionID();
            if (logger.isActivated()) {
                logger.info("FTS Receive file transfer invitation: "+ "Chatsessionid: " + chatSessionId);
            }
        ...
        synchronized(lock) {
            ...
            for (int i=0; i < N; i++) {
                try {
                    ...
                    listeners.getBroadcastItem(i).onNewFileTransferReceived(session.getSessionID(),session.shouldAutoAccept(),isGroup,chatSessionId,chatId,timeLen);
                } catch(Exception e) {
                    if (logger.isActivated()) {
                        logger.error("FTS Can't notify listener", e);
                    }
    ...
    public void receiveFileTransferInvitation(FileSharingSession session, boolean isGroup) {
		if (logger.isActivated()) {
			logger.info("Receive file transfer invitation from " + session.getRemoteContact() + " Timelen " + session.getTimeLen());
		}
		String chatId = null;
		...
		if ((isGroup) && (!isBurn)) {
			...
			// Update rich messaging history
    		RichMessagingHistory.getInstance().addBurnFileTransfer(number, session.getSessionID(), FileTransfer.Direction.INCOMING, session.getContent());
		}
		else{
			// Update rich messaging history
    		RichMessagingHistory.getInstance().addFileTransfer(number, session.getSessionID(), FileTransfer.Direction.INCOMING, session.getContent());
		...
		if(session.getTimeLen() > 0) {
			RichMessagingHistory.getInstance().updateFileTransferDuration(session.getSessionID(),session.getTimeLen());
		}
		...
		if(RcsSettings.getInstance().isCPMSupported()) {
        	if(!(RichMessagingHistory.getInstance().getCoversationID(session.getRemoteContact(), 1).equals(session.getConversationID()))){
        		if(session.getConversationID() != null) {
	        		if (logger.isActivated()) {
	        	       logger.info("FTS receiveFileTransferInvitation  OldId: "  + RichMessagingHistory.getInstance().getCoversationID(session.getRemoteContact(),1) + " NewId: " + session.getConversationID());
	        	    }
	                RichMessagingHistory.getInstance().UpdateCoversationID(session.getRemoteContact(),session.getConversationID(), 1);
        		} else {
        			if (logger.isActivated()) {
		...
		//addFileTransferPauseResumeData(session.getSessionID(),session.getContent().getUrl(),session.getContent().getSize(),0, session.getRemoteContact(),session.getHashselector());
	        ...
	        for (int i=0; i < N; i++) {
	            try {
	            	if(isPublicAccountFile){
		            	 listeners.getBroadcastItem(i).onNewPublicAccountChatFile(session.getSessionID(),session.shouldAutoAccept(),isGroup,chatSessionId,chatId);
	            	} else {
		            	if(isBurn){                                
                                
		            		listeners.getBroadcastItem(i).onNewBurnFileTransfer(session.getSessionID(),isGroup,chatSessionId,chatId);	
		            	}else{
	                        listeners.getBroadcastItem(i).onNewFileTransfer(session.getSessionID());
		            		listeners.getBroadcastItem(i).onNewFileTransferReceived(session.getSessionID(),session.shouldAutoAccept(),isGroup,chatSessionId,chatId,timeLen);
		            	}
	 ...
	 * @param isGroup is group file transfer
	 */
    public void receiveResumeFileTransferInvitation(FileSharingSession session, boolean isGroup, String fileTransferId) {
		if (logger.isActivated()) {
		...
		if (session instanceof HttpFileTransferSession) {
			...
			chatId = ((TerminatingGroupFileSharingSession)session).getGroupChatSession().getContributionID();
			if (logger.isActivated()) {
				logger.info("Receive Resume group MSRP file transfer invitation chatid: " + chatId);
			}
		}
        ...
        /** @} */
    	  if (session instanceof HttpFileTransferSession) {
    		...
    		if (logger.isActivated()) {
				logger.info("Resume file transfer invitation: "+ "Chatsessionid: " + chatSessionId);
			}
			intent.putExtra("chatSessionId", chatSessionId);
    	    intent.putExtra("chatId", chatId);
    	    intent.putExtra("isGroupTransfer", isGroup);
	...
	public void receiveFileTransferInvitation(FileSharingSession session, ChatSession chatSession) {
		...
		if (logger.isActivated()) {
			logger.info("FTS Receive file transfer invitation from1 " + session.getRemoteContact());
		}
		// Update rich messaging history
...
/* TODO		receiveFileTransferInvitation(session, chatSession.isGroupChat());
	...
	public static PauseResumeFileObject getPauseInfo(String transferId){
		...
		for(int j=0; j < mFileTransferPauseResumeData.size(); j++) {
    		PauseResumeFileObject object = FileTransferServiceImpl.mFileTransferPauseResumeData.get(j);
    		if(object.mFileTransferId.equals(transferId))
    		{
    			if (logger.isActivated()) {
    	    		logger.info("getPauseInfo, FileTransfer Id & currentBytesTransferred" + object.mFileTransferId);
    	    	}
    			return object;
    		}
        }
	 ...
	 public static PauseResumeFileObject getHashPauseInfo(String hashSelector){
		 ...
		 if (logger.isActivated()) {
			...
			for(int j=0; j < mFileTransferPauseResumeData.size(); j++) {
	    		...
	    		{
	    			if (logger.isActivated()) {
	    	    		logger.info("getPauseInfo, hashSelector Id & currentBytesTransferred" + object.hashSelector);
	    	    	}
	    			return object;
	    		}
	        }
     ...
     */
    public IFileTransfer transferMedia(String contact, String filename, String fileicon, IFileTransferListener listener ,int timeLen) throws ServerApiException {
        if (logger.isActivated()) {
            logger.info("FTS Transfer media file " + filename + " to " + contact + "Timelen " + timeLen);
        }
...
public static void addFileTransferPauseResumeData(String fileTransferId, String path, long size , long bytesTransferred,String contact,String hash)
 {
		if (logger.isActivated()) {
			logger.info("addFileTransferPauseResumeData fileTransferId "
					+ fileTransferId + " path : " + path + "size : " + size
					+ "bytesTransferred :" + bytesTransferred + "contact :" + contact
					+ "hash:" + hash);
		}
...
public IFileTransfer resumeGroupFileTransfer(String chatId, String fileTranferId, IFileTransferListener listener) throws ServerApiException {
    if (logger.isActivated()) {
    ...
    try {
        ...
        List fileParticipants = RichMessagingHistory.getInstance().getFileTransferContacts(fileTranferId);
        if (logger.isActivated() && fileParticipants!= null) {
            logger.info("resumeGroupFile file participants:" + fileParticipants + " size:" + fileParticipants.size());
        }               
        session = Core.getInstance().getImService().initiateGroupFileTransferSession(fileParticipants, content, null,chatId,null);
        ...
        if(direction.equals("1"))
        RichMessagingHistory.getInstance().updateFileTransferId(fileTranferId, sessionApi.getTransferId());
        // Start the session
        Thread t = new Thread() {
            public void run() {
...
public IFileTransfer resumePublicFileTransfer(String fileTranferId, IFileTransferListener listener, int timeLen) throws ServerApiException {
    if (logger.isActivated()) {
    ...
    try {
        ...
        Core.getInstance().getImService().initiatePublicFileTransferSession(contact, content, null, null, null); // TODO
     ...
     * @throws ServerApiException
     */
    public IFileTransfer resumeFileTransfer(String fileTranferId, IFileTransferListener listener) throws ServerApiException {
		if (logger.isActivated()) {
		...
		try {
			...
			List fileParticipants = RichMessagingHistory.getInstance().getFileTransferContacts(fileTranferId);
			if (logger.isActivated() && fileParticipants!= null) {
	            logger.info("resumeFile file participants:" + fileParticipants + " size:" + fileParticipants.size());
	        }
			...
			if(fileParticipants != null && fileParticipants.size() > 1) 
			    session = Core.getInstance().getImService().initiateMultiFileTransferSession(listofParticipant, content, false, null, null);
			else
                session = Core.getInstance().getImService().initiateFileTransferSession(contact, content, null, null, null); // TODO
			final FileSharingSession resumedSession = session;
			resumedSession.fileTransferPaused();
			resumedSession.setOldFileTransferId(fileTranferId);
			resumedSession.setSessionId(fileTranferId);
			...
			if (pauseResumeObject != null && direction.equals("1")) {
				if (logger.isActivated()) {
					logger.info("resumeFile file pauseResumeObject:"
							+ pauseResumeObject +" & new Id is" + sessionApi.getTransferId());
				}
				pauseResumeObject.mFileTransferId = sessionApi.getTransferId();
    ...
    public IFileTransfer transferFile(String contact, String filename, String fileicon, IFileTransferListener listener) throws ServerApiException {
		...
		try {
            ...
            addFileTransferPauseResumeData(session.getSessionID(),filename,session.getContent().getSize(),0,contact,"");
			// Start the session
	        Thread t = new Thread() {
	    		public void run() {
	    			session.startSession();
     ...
     * @throws ServerApiException
     */
    public IFileTransfer transferGeoLocFile(String contact, String filename, String fileicon, IFileTransferListener listener) throws ServerApiException {
        if (logger.isActivated()) {
     ...
     * @throws ServerApiException
     */
    public IFileTransfer transferPublicChatFile(String contact, String filename, String fileicon, IFileTransferListener listener,int timeLen) throws ServerApiException {
        if (logger.isActivated()) {
            logger.info("transferGeoLocFile file " + filename + " to " + contact);
        }
        ...
        try {
            ...
            FileDescription desc = FileFactory.getFactory().getFileDescription(filename);
            MmContent content = ContentManager.createMmContentFromUrl(filename, desc.getSize());
            final FileSharingSession session = Core.getInstance().getImService().initiatePublicFileTransferSession(contact, content, fileicon, null, null); // TODO
            session.setPublicChatFile(true);
            session.setTimeLen(timeLen);
            // Add session listener
            FileTransferImpl sessionApi = new FileTransferImpl(session);
     ...
     * @throws ServerApiException
     */
    public IFileTransfer transferLargeModeFile(String contact, String filename, String fileicon, IFileTransferListener listener) throws ServerApiException {
        ...
        try {
            ...
            FileDescription desc = FileFactory.getFactory().getFileDescription(filename);
            MmContent content = ContentManager.createMmContentFromUrl(filename, desc.getSize());
            final FileSharingSession session = Core.getInstance().getImService().initiateLargeModeFileTransferSession(contact, content, fileicon, null, null); // TODO
    ...
    public IFileTransfer transferBurnFile(String contact, String filename, String fileicon, IFileTransferListener listener) throws ServerApiException {
		if (logger.isActivated()) {
		...
		try {
			...
			// Update rich messaging history
			RichMessagingHistory.getInstance().addBurnFileTransfer(contact, session.getSessionID(), FileTransfer.Direction.OUTGOING, session.getContent());
    ...
    public IFileTransfer transferLargeModeBurnFile(String contact, String filename, String fileicon, IFileTransferListener listener) throws ServerApiException {
     ...
     * @throws ServerApiException
     */
    public IFileTransfer transferFileToMultirecepient(List<String> contacts, String filename, boolean fileicon, IFileTransferListener listener , int timeLen) throws ServerApiException {
        if (logger.isActivated()) {
        ...
        try {
            ...
            FileDescription desc = FileFactory.getFactory().getFileDescription(filename);
            MmContent content = ContentManager.createMmContentFromUrl(filename, desc.getSize());
            final FileSharingSession session = Core.getInstance().getImService().initiateMultiFileTransferSession(new ListOfParticipant(contacts), content, fileicon, null, null); // TODO
            ...
            RichMessagingHistory.getInstance().addFileTransfer(newContact, session.getSessionID(), FileTransfer.Direction.OUTGOING, content);
            if(session.getTimeLen() > 0) {
    			RichMessagingHistory.getInstance().updateFileTransferDuration(session.getSessionID(),session.getTimeLen());
    		}
            addFileTransferPauseResumeData(session.getSessionID(),filename,session.getContent().getSize(),0,"","");
            // Update rich messaging history
            //RichMessagingHistory.getInstance().addOutgoingMultiFileTransfer(session.getSessionID(), content, null, contacts);
	 ...
	 * @throws ServerApiException
	 */
	public IFileTransfer transferFileToGroup(String chatId, List<String> contacts, String filename, String fileicon,int timeLen,IFileTransferListener listener) throws ServerApiException {
		if (logger.isActivated()) {
	...
	public void initiateFileSpamReport(String contact, String FtId) {
		...
		final OriginatingFileSpamSession session = new OriginatingFileSpamSession(Core.getInstance().getImService(), PhoneUtils.formatNumberToSipUri("+861008699999"), FtId);
    ...
    public void handleFileDeliveryStatus(String ftSessionId, String status,String contact) {
    	if (logger.isActivated()) {
            logger.info("FTS handleFileDeliveryStatus contact: " + contact + " FtId: " + ftSessionId + " Status: " + status);
        }
