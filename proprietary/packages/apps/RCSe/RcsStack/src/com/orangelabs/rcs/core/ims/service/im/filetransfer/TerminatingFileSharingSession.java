...
public class TerminatingFileSharingSession extends ImsFileSharingSession implements MsrpEventListener {
	...
	public TerminatingFileSharingSession(ImsService parent, SipRequest invite) {		
		...
		if(LauncherUtils.supportOP01()) {
			...
			if (logger.isActivated()) {
	    		logger.info("TerminatingFileSharingSession1" + messgeId + ";Instance:" + remoteInstanceId);
	    	}
		}

		// Create dialog path
		createTerminatingDialogPath(invite);
		String name ="thumb_" + getContent().getName();
		String mimeType = MediaFile.getMimeTypeForFile(name);		
		if (logger.isActivated()) {
    		logger.debug("FTS Thumb Extraction, mimetype and name are : "+ mimeType + ":" + name);
   		}
		if (mimeType != null && mimeType.contains("video")) {
			if(mimeType.contains("3gpp"))
				name = name.replace(".3gp", ".jpg");
			else if(mimeType.contains("mpeg-4"))
	...
	// Init file
		...
		if (logger.isActivated()) {
	   		logger.error("FTS lemghth of thumnail file is" + length + "URL is" + getThumbUrl());
		}
        ...
        isBurnMsg = SipUtils.isFeatureTagPresent(invite, FeatureTags.FEATURE_RCSE_CPM_BURN_AFTER_READING_FEATURE_TAG);
        ...
        if (logger.isActivated()) {
            logger.debug("FTS autoAccept: " + autoAccept + " lessWarnSize: "
                    + lessWarnSize + " enable: " + isRoaming +"mAutoAccept" + mAutoAccept + " , isburnmessage : "+isBurnMsg);
        }
	...
	public void run() {
		try {
		    ...
		    if (logger.isActivated()) {
        		logger.debug("FTS terminating file session isSendonly: " + isSendOnly() + " isPaused" + isFileTransferPaused());
        	}
		    if (logger.isActivated()) {
        		logger.debug("FTS terminating file session isSecureProtocolMessage: " + isSecureProtocolMessage());
        	}
    ...
    private void sendDeliveryReport(String status) {
		...
		if (messgeId != null) {
            ...
            ((InstantMessagingService) getImsService()).getImdnManager().sendMessageDeliveryStatusImmediately(
                    getRemoteContact(), messgeId, status, remoteInstanceId);
		}
    }
    ...
    public void prepareMediaSession() throws Exception {
    	...
    	if (logger.isActivated()) {
        	logger.info("FTS prepareMediaSession isSendOnly: " + this.isSendOnly() + " isPaused: " + this.isFileTransferPaused());
        }
    ...
    public void startMediaSession() throws Exception {
        // Nothing to do in terminating side
    	if (logger.isActivated()) {
        	logger.info("FTS startMediaSession isSendOnly: " + this.isSendOnly() + " isPaused: " + this.isFileTransferPaused());
        }
    	if(this.isSendOnly() && this.isFileTransferPaused()){
    		if (logger.isActivated()) {
    ...
    public void startResumeMediaSession() throws Exception {
        ...
        try {
            ...
            {            	
            	bytesTransferredtoSkip = FileTransferServiceImpl.getPauseInfo(oldFileTransferId).bytesTransferrred;
    ...
    private void deleteFile() {
        if (logger.isActivated()) {
            logger.debug("FTS Delete incomplete received file Paused = " + fileTransferPaused);
        }
        try {
