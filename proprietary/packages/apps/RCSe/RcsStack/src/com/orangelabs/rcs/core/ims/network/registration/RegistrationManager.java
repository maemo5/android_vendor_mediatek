...
public class RegistrationManager extends PeriodicRefresher {
     ...
     * Dialog path
     */
    private SipDialogPath dialogPathSubscription = null;
    ...
    public synchronized void subscription() {
        ...
        try {
            // Create a dialog path if necessary
            if (dialogPathSubscription == null) {
                ...
                // Set target
                String target = RcsSettings.getInstance().getImConferenceUri();
                ...
                // Create a dialog path
                dialogPathSubscription = new SipDialogPath(
                        networkInterface.getSipManager().getSipStack(),
                        callId,
                        1,
                        target,
                ...
                // Increment the Cseq number of the dialog path
                dialogPathSubscription.incrementCseq();
            }

            // Create Subscription request
            SipRequest subscription = SipMessageFactory.createCpimChatListSubscribe(dialogPathSubscription,
                    3600,
                    0);
            ...
            if (logger.isActivated()) {
                logger.error("Subscription has failed", e);
            }
     ...
     * 
     * @param subscription SIP SUBSCRIPTION
     * @throws SipException
     * @throws CoreException
     */
    private void sendSubscription(SipRequest subscription) throws Exception {
         /**
         * M: Acquire WakeLock @{
         */
        acquirePeriodWakeLock();
         ...
         */
        if (logger.isActivated()) {
            logger.info("Send SUBSCRIPTION, expire=" + subscription.getExpires());
        }
        /**
         * M: add WakeLock @{
         */
        ...
        try {
         ...
         // Send SUBSCRIPTION request
         ctx = networkInterface.getSipManager().sendSipMessageAndWait(subscription);
        ...
        // Wait response
        if (logger.isActivated()) {
            logger.info("Wait response subscription");
        }
        if (ctx != null) {
            ctx.waitResponse(SipManager.TIMEOUT);
        }
        ...
        if (ctx.isSipResponse()) {
            ...
            if (ctx.getStatusCode() == 200) {
                if (logger.isActivated()) {
                    logger.info("200 OK response received for subscription");
                }
 
            } else {
                // Other error response
                if (logger.isActivated()) {
                    logger.info("response received for subscription:"+ ctx.getStatusCode());
                }   
            }
            } finally {
                releaseNetworkWakeLock();
            ...
            if (logger.isActivated()) {
                logger.info("no response received for subscription");
            }  
        }
    } 
