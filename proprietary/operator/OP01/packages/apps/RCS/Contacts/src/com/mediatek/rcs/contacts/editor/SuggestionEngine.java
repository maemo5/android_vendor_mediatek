...
public class SuggestionEngine extends HandlerThread {
    ...
    private void loadRawContacts(long contactId, int index) {
        ...
        try {
            ...
            if (cursor != null) {
                ...
                if (list == null) {
                    Log.i(TAG, "filterCursor new RawContact list: " + index);
                ...
                while (cursor.moveToNext()) {
                    ...
                    if (!containsRawContact(list, rawContactId)) {
                        Log.i(TAG, "filterCursor add RawContact: " + rawContactId);
    ...
    private void loadSuggestions(SearchData searchData) {
        ...
        //String[] selectArgs = new String[] { ACCOUNT_TYPE_LOCAL_PHONE, String.valueOf(contactId) };
        String[] selectArgs;
        if (numberE164 == null) {
            selectArgs = new String[] { ACCOUNT_TYPE_LOCAL_PHONE, number };
        } else {
            selectArgs = new String[] { ACCOUNT_TYPE_LOCAL_PHONE, number, numberE164 };
        }
        Cursor cursor = null;
