...
public class RcsProxyReceivers extends IpEmptyReceiver {
    
    public static final String TAG = "RcsProxyReceivers";
    ...
    private void asyncOnReceive(Context context, Intent intent) {
        ThreadMapCache.createInstance(context);
        ...
        try {
            if (action.equalsIgnoreCase(IpMessageConsts.ServiceNotification.BROADCAST_RCS_NEW_MESSAGE)) {
                listener.onNewMessage(smsId);
            } else if (action.equalsIgnoreCase(IpMessageConsts.ServiceNotification.BROADCAST_RCS_GROUP_NEW_MESSAGE)) {
                listener.onNewGroupMessage(chatId, smsId, contact);
            } else if (action.equalsIgnoreCase(IpMessageConsts.ServiceNotification.BROADCAST_RCS_GROUP_BEEN_KICKED_OUT)) {
                listener.onMeRemoved(chatId, contact);
            } else if (action.equalsIgnoreCase(IpMessageConsts.ServiceNotification.BROADCAST_RCS_GROUP_ABORTED)) {
                listener.onAbort(chatId);
            } else if (action.equalsIgnoreCase(IpMessageConsts.ServiceNotification.BROADCAST_RCS_GROUP_INVITATION)) {
                String subject = intent.getStringExtra(IpMessageConsts.ServiceNotification.KEY_SUBJECT);
                Participant participant = (Participant)intent.getParcelableExtra(IpMessageConsts.ServiceNotification.KEY_PARTICIPANT);
