...
import com.mediatek.rcs.message.ft.FTNotificationsManager;
...
*/
        ...
        /**
         * onAddSentFileTransfer, add sentFileTransfer into mPendingList and checknext
         * 
         */
      ...
      /*
        ...
        */
        ...
        /**
         * resendFileTransfer and call resumeFileTransfer()
         * 
         */
         /*
      ...
      public void resendFileTransfer(SentFileTransfer sentFileTransfer, boolean haveInstance) {
            ...
            if (sentFileTransfer != null) {
                ...
                Log.d(TAG, "resendFileTransfer, mPendingList.size() after add = "+ mPendingList.size());
   ...
   /*
    ...
    public static class SentFileTransfer {
        ...
        public class StatusListener implements FileTransferManager.INotificationsListener {
            ...
            public void notificationsReceived(Intent intent) {
                if(intent.getAction() == FTNotificationsManager.ACTION_STATUS_CHANGE){
                    boolean status = (boolean)intent.getExtras().get(FTNotificationsManager.KEY_STATUS);
                    if (status == true) {
                        //resend it
        ...
        public SentFileTransfer(String contact, String filePath, boolean needSaveDb, boolean isBurn, String fid) {
            ...
            mChatType = ONE2ONE;
            mBurnType = isBurn;
            mDuration = RCSUtils.getDuration(ContextCacher.getHostContext(),filePath); //get duration form filepath
        ...
        public SentFileTransfer(Set<String> contacts, String filePath, String chatId, String chatSessionId, boolean needSaveDb, long dummyId) {
            ...
            mChatType = GROUP;
            mBurnType = false;          
            mDuration = RCSUtils.getDuration(ContextCacher.getHostContext(),filePath);
            mContacts = contacts;
            mChatId = chatId;
            ...
            if (instance != null && (mChatType == ONE2ONE || mChatType == ONE2MULTI)) {
                ...
                try {
                    ...
                    Log.d(TAG, "resumeGroupFileTransfer,  mChatSessionId = " + mChatSessionId);
                     mFileTransferObject =
                         fileTransferService.resumeGroupFileTransfer(mChatSessionId,mFileTransferTag,mFileTransferListener);
                } catch (JoynServiceException e) {
                    Log.e(TAG, "Can't get fileTransferService! ");
                }
            }
...
/*
  ...
  */
    ...
    protected class ReceiveFileTransferManager {
         ...
         */
        public synchronized void addReceiveFileTransfer(FileTransfer fileTransferObject, boolean isBurn, String chatId, boolean isGroup, String chatSessionId) {
            ...
            if (null != fileTransferObject) {
                ...
                new ReceiveFileTransfer(fileTransferObject,isBurn,chatId,isGroup,chatSessionId);
        ...
        public synchronized void cancelReceiveFileTransfer() {
            ...
            ArrayList<ReceiveFileTransfer> tempList = new ArrayList<ReceiveFileTransfer>(mActiveList);
        ...
        public void cancelFileTransfer(String targetFileTransferTag) {
            Log.d(TAG, "cancelFileTransfer, begin to cancel receive file transfer with tag, " + targetFileTransferTag);
            ReceiveFileTransfer receiveFileTransfer = findFileTransferByTagFromActiveList(targetFileTransferTag);
    ...
    protected class ReceiveFileTransfer {
        ...
        public ReceiveFileTransfer(FileTransfer fileTransferObject, boolean isBurn, String chatId, boolean isGroup, String chatSessionId) {
            if (null != fileTransferObject) {
                Log.d(TAG, "ReceiveFileTransfer() constructor FileTransfer is "
                        + fileTransferObject);
                
                handleReceiveFileTransferInvitation(fileTransferObject, chatId, isGroup,isBurn,chatSessionId);
            }
        }
        ...
        protected void handleReceiveFileTransferInvitation(FileTransfer fileTransferObject, String chatId, boolean isGroup, boolean isBurn, String chatSessionId) {
        ...
        protected void reAcceptFileTransferInvitation() {
            ...
            if (instance != null && mChatType == ONE2ONE) {
                ...
                Log.w(TAG, "reAcceptFileTransferInvitation in GROUP, mChatSessionId = " + mChatSessionId);
                try {
                    fileTransferService = instance.getFileTransferApi();
                     mFileTransferObject =
                     fileTransferService.resumeGroupFileTransfer(mChatSessionId,mFileTransferTag,mFileTransferListener);
...
/*
    ...
    public void handleResendFileTransfer(long ipMsgId) {
        ...
        if (isDummyId(-ipMsgId)) {
            ...
            if (ftInfo.contacts == null && ftInfo.contact != null) {
                ...
                Log.d(TAG, "Get dummy sentfileTransfer from Resendable List, need remove it from  Resendable List");
    ...
    public boolean handleFileTransferInvitation(String fid, boolean isBurn, String chatId, boolean isGroup, String chatSessionId) {
        Log.w(TAG, "handleFileTransferInvitation() entry, fid = " + fid + "isBurn = " + isBurn + "isGroup = " + isGroup + "chatSessionId = " + chatSessionId);
        ...
        if (instance != null) {
            ...
            if (fileTransferService != null) {
                try {
                    ...
                    mReceiveFileTransferManager.addReceiveFileTransfer(fileTransferObject,isBurn,chatId,isGroup,chatSessionId);
                    return true;
                } catch (JoynServiceException e) {
                    Log.e(TAG,
    ...
    public void handleReAcceptFileTransferInGroup(String fileTransferTag, long ipMsgId, String chatSessionId) {
        Log.w(TAG, "handleReAcceptFileTransferInGroup() entry...");
        Log.w(TAG, "fileTransferTag = " + fileTransferTag + "ipMsgId = " + ipMsgId + "chatSessionId = " + chatSessionId);
        ReceiveFileTransfer receiveFileTransfer = 
                mReceiveFileTransferManager.findFileTransferByTagFromActiveList(fileTransferTag);
        if (null != receiveFileTransfer) {
            Log.w(TAG, "null != receiveFileTransfer, call reAcceptFileTransferInvitation directly");
    ...
    public void handleFileTransferDeliveryStatus(Intent invitation) {
        ...
        Log.d(TAG, "handleFileTransferDeliveryStatus() from broadcast, msgId: " + fileTransferTag
                + ", status: " + status);
        FileTransferChatWindow.onReceiveMessageDeliveryStatus(fileTransferTag, status);
        return;
    }
