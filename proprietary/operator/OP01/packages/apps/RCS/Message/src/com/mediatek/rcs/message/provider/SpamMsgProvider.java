...
public class SpamMsgProvider extends ContentProvider {
    ...
    public int delete(Uri uri, String where, String[] whereArgs) {
        ...
        count = deleteSpamMsgs(db, finalSelection, whereArgs);
        if (count > 0) {
            notifyChange();
        }
        return count;
    ...
    private int deleteSpamMsgs(SQLiteDatabase db, String where, String[] whereArgs) {
        int count = 0;
        Cursor cursor = null;
    ...
    private int deleteIpMsgByIds(Uri uri, Set<Long> ids) {
        return getContext().getContentResolver().delete(uri, "_id " + formatInClause(ids), null);
    }
