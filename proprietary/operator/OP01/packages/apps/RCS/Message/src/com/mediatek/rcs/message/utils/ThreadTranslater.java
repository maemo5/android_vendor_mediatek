...
public final class ThreadTranslater {
   ...
   public static void saveThreadandTag(Long threadId, String tag) {
       Log.d(TAG, "saveThreadandTag() entry threadId is " + threadId + " tag is " + tag);
   ...
   public static String translateThreadId(Long threadId) {
       Log.d(TAG, "translateThreadId() entry threadId is " + threadId);
       String result = THREAD_CACHE.get(threadId);
       Log.d(TAG, "translateThreadId() result is " + result);
       return result;
   }
   
   public static Long translateTag(String tag) {
       Log.d(TAG, "translateThreadId() entry threadId is " + tag);
       Long threadID = 0L;
   ...
   public static boolean tagExistInCache(String tag) {
       Log.d(TAG, "tagExistInCache() entry tag is " + tag);
       if (TextUtils.isEmpty(tag)) {
           Log.d(TAG, "tagExistInCache() tag is null ");
           return false;
       }
       Map<Long, String> map = THREAD_CACHE.snapshot();
       boolean exist = map.containsValue(tag);
       Log.d(TAG, "tagExistInCache() exist is " + exist);
       return exist;
   }
}
