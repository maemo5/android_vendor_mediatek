...
import com.mediatek.rcs.message.ft.FileTransferManager.INotificationsListener;
...
class FTNotificationsManager {
    ...
    public void registerNotificationsListener(INotificationsListener notiListener) {
        synchronized (LISTENER_SYNC) {
            Log.d(TAG, "FTNotificationsManager,registerNotificationsListener,listener = " + notiListener);
            if (sNotificationsListeners.contains(notiListener)) {
                Log.d(TAG, "FTNotificationsManager,registerNotificationsListener, already contains.");
                return;
    ...
    public void unregisterNotificationsListener(INotificationsListener notiListener) {
        synchronized (LISTENER_SYNC) {
            Log.d(TAG, "FTNotificationsManager, unregisterNotificationsListener, listener = " + notiListener);
            if (!sNotificationsListeners.contains(notiListener)) {
                Log.d(TAG, "FTNotificationsManager,unregisterNotificationsListener, do not contain !!");
                return;
            }
