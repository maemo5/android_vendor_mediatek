...
public class EmojiImpl {
    ...
    private void parseEmojiIcon(Context context, SpannableString input, Pattern patten, int size) throws Exception {
    ...
    private void parseEmojiString(Context context, StringBuilder input, Pattern patten) throws Exception {
        Matcher matcher = patten.matcher(input);
        while (matcher.find()) {
            String key = string2unicode(matcher.group());
    ...
    //
    public CharSequence getEmojiExpression(CharSequence chars, boolean showIcon, int start, int length, int size){
        Log.d(TAG, "getEmojiExpression chars=" + chars + " showIcon=" + showIcon + " start=" + start + " length="+ length);
        if (chars == null) {
            return null;
        }
