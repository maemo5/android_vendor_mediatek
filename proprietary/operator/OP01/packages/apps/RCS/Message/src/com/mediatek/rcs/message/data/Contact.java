...
//import com.mediatek.mms.ipmessage.IpMessageConst;

public class Contact {
    public static final String GROUP_START = "7---";
    public static final String JOYN_START = "9+++";
    public static final String LogTagAPP = "Mms.app";
    ...
    public static final String LogTagCONTACT = "Mms.contact";
...
//            if (Log.isLoggable(LogTagAPP, Log.VERBOSE)) {
...
//    };
    ...
    public synchronized void setNumber(String number) {
         ...
         */
        /// M: @{
        //Log.d(M_TAG, "setNumber(): before number=" + number);
        mNumberProtosomatic = number;
        mNumber = getValidNumber(number);
        //Log.d(M_TAG, "setNumber(): after number=" + mNumber);
        /// @}
    ...
    private static class ContactsCache {
        ...
        private Contact get(String number, boolean isMe, boolean canBlock) {
            if (Log.isLoggable(LogTagCONTACT, Log.DEBUG)) {
            ...
            //Log.d(M_TAG, "get(" + number + ", " + isMe + ", " + canBlock + "): mWaitTime = " + mWaitTime);
            if (r != null) {
                if (canBlock) {
                    /// M: @{
                    pushTask(r);
        ...
        private void updateContact(final Contact c) {
            if (c == null) {
                /// M:
                Log.d(TAG, "Contact.updateContact(): Contact for query is null.");
                return;
            }
            ...
            synchronized (c) {
                if (contactChanged(c, entry)) {
                    ...
                    /// M: add for ipmessage
                    if (c.mThreadId > 0) {
                        Log.d("avatar", "Contact.updateContact(): c.mThreadId > 0, c.mName = " + c.mName);
                        if (TextUtils.isEmpty(c.mName) || c.mNumber.equals(c.mName)) {
                            //c.mName = (new ContactManager(mContext)).getNameByThreadId(c.mThreadId);
                            Log.d("avatar", "Contact.updateContact(): group name = " + c.mName);
                        }
                    } else {
                        c.mName = entry.mName;
                    }
        ...
        private Contact getContactInfoForPhoneNumber(String number) {
            ...
            //Log.d(M_TAG, "getContactInfoForPhoneNumber(): isValidNumber=" + isValidNumber
            //        + ", number=" + number);
            /// @}
...
//
//                //Log.d(M_TAG, "getContactInfoForPhoneNumber(): exact query number=" + normalizedNumber);
...
//                        Log.d(M_TAG, "It isn't a E164 number : " + number);
//                        if (cursor != null) {
//                            cursor.close();
//                        }
...
//                /// @}
            ...
            if (number.startsWith(JOYN_START)) {
        ...
        private void fillPhoneTypeContact(final Contact contact, final Cursor cursor) {
            ...
            if (//MmsConfig.isServiceEnabled(mContext) &&
                contact.getNumber().startsWith(GROUP_START)) {
                return;
            }
        ...
        private byte[] loadAvatarData(Contact entry) {
            ...
            if ((!entry.mIsMe && entry.mPersonId == 0) || entry.mAvatar != null || !sNeedLoadAvatar) {
                /// M:
                Log.d(TAG, "loadAvatarData(): return null");
                return null;
            }
            ...
            /// M:
            Log.d(TAG, "loadAvatarData(): contactUri=" + contactUri);
            InputStream avatarDataStream = Contacts.openContactPhotoInputStream(
                        mContext.getContentResolver(),
                        contactUri);
            try {
                /// M:
                Log.d(TAG, "loadAvatarData(): avatarDataStream is null?=" + (avatarDataStream == null));
        ...
        private Contact getContactInfoForEmailAddress(String email) {
            /// M:
            Log.d(TAG, "getContactInfoForEmailAddress(): email=" + email);
            ...
            if (cursor != null) {
                /// M:
                Log.d(TAG, "getContactInfoForEmailAddress(): cursor != null, cursor.getCount()=" + cursor.getCount());
                try {
                    while (cursor.moveToNext()) {
                        boolean found = false;
                        synchronized (entry) {
                        ...
                        /// M:
                        Log.d(TAG, "getContactInfoForEmailAddress(): found=" + found);
                        if (found) {
        ...
        //synchronized (ContactsCache.this) {
                ...
                //Log.d(M_TAG, "Contact.internalGet(): workingNumberOrEmail=" + workingNumberOrEmail);
                String key = "";
                /// @}

                final boolean isNotRegularPhoneNumber = isMe || Mms.isEmailAddress(numberOrEmail) ||
                        isAlias(numberOrEmail);
                /// M: @{
                if (isNotRegularPhoneNumber) {
                    // Log.d(M_TAG, isMe ? "Contact.internalGet(): It's me!" :
                    // "Contact.internalGet(): It's a Email!");
                    key = numberOrEmail;
                } else if (isWellFormedSmsAddress(workingNumberOrEmail)) {
                    //Log.d(M_TAG, "Contact.internalGet(): Number without space and '-' is a well-formed number for sending sms.");
                    if (workingNumberOrEmail.length() > NORMAL_NUMBER_MAX_LENGTH) {
                        Log.d(TAG, "Contact.internalGet(): Long number.");
                        /// M: handle number like 1252002613111111111
                        key = workingNumberOrEmail;
                    } else {
                        //Log.d(M_TAG, "Contact.internalGet(): Normal number.");
                ...
                //Log.d(M_TAG, "Contact.internalGet(): key=" + key);
                /// @}
        ...
        private String getKey(String numberOrEmail, boolean isMe) {
            ...
            Log.d(TAG, "getKey(" + numberOrEmail + ", " + isMe + ") return:" + key);
            return key;
        }
                ...
                for (ArrayList<Contact> alc : mContactsHash.values()) {
                    for (Contact c : alc) {
                        synchronized (c) {
                            ...
                            if (number != null && number.startsWith(GROUP_START)) {
                                c.clearAvatar();
                            }
                        }
                    }
    ...
    public static String getValidNumber(String numberOrEmail) {
        ...
        //Log.d(M_TAG, "Contact.getValidNumber(): numberOrEmail=" + numberOrEmail);
        /// M: add for ipmessage
        if (//MmsConfig.isServiceEnabled(MmsApp.getApplication()) &&
            numberOrEmail.startsWith(GROUP_START)) {
    ...
    //     Log.d(M_TAG, "Contact.getGroupAvatar(): mAvatar is null ?= "
    //             + (mAvatar == null));
    //     if (mAvatar == null) {
    //         final Object lock = new Object();
    //         mIpMessageAvatarBitmap = null;
