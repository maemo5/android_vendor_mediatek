...
public class ProfileService extends JoynService {
    ...
    private final static String ACTION_BIND_PROFILESERVICE = "com.mediatek.rcs.contacts.ACTION_BIND_PROFILESERVICE";
    ...
    private static final int OP_GET_QRCODE = 4;
    private static final int OP_SET_QRCODE_MODE = 5;
    private static final int OP_GET_QRCODE_MODE = 6;

    //private static final int RSP_GET_PROFILE = 5;
    //private static final int RSP_SET_PROFILE = 6;
    ...
    private static final int NOTIFY_TYPE_SET_PROFILE = 1;
    private static final int NOTIFY_TYPE_GET_CONTACT_PORTRAIT = 2;
    private static final int NOTIFY_TYPE_GET_QRCODE = 3;
    private static final int NOTIFY_TYPE_SET_QRCODE_MODE = 4;
    private static final int NOTIFY_TYPE_GET_QRCODE_MODE = 5;
    ...
    private void handleSetProfile (HashMap profile) {
        ...
        try {
           mPSM.setProfileInfo(profileInfo);
        } catch (RemoteException re) {
            re.printStackTrace();
        }
    }
    ...
    public void connect(){
        ...
        if (mPSM == null) {
            try {
                ...
                intent.setClassName("com.mediatek.rcs.contacts", "com.mediatek.rcs.contacts.profileservice.ProfileServiceManager");
    ...
    public void setProfileInfo (HashMap Profile) {
    ...
    private void notifyProfileListener(int notifyType, int result){
        ...
        for (int i=0; i < mListeners.size(); i++){
            if (notifyType == NOTIFY_TYPE_GET_PROFILE) {
                mListeners.get(i).onGetProfile(result);
            } else if (notifyType == NOTIFY_TYPE_SET_PROFILE) {
    ...
    private void notifyPortraitListener(int result, String portrait, String number, String mimeType){  
    ...
    private void notifyQRCodeListener(int notifyType, int result, int mode){  
        Log.d(TAG, "notifyQRCodeListener, result: " + result + " mode: " + mode + " notifyType: " + notifyType);
