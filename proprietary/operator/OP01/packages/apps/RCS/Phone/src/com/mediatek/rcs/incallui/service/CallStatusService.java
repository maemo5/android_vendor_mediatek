...
public class CallStatusService extends Service {
    ...
    private final RemoteServiceCallbackList<IServiceMessageCallback> mMessageCallback = new RemoteServiceCallbackList<IServiceMessageCallback>();
    private final RemoteServiceCallbackList<IServicePresenterCallback> mPresenterCallback = new RemoteServiceCallbackList<IServicePresenterCallback>();
    ...
    private final ICallStatusService.Stub mBinder = new ICallStatusService.Stub() {
        ...
        public void notifyToClient(String name, String status, String time) {
            try {
                ...
                Xlog.i(TAG, "notifyToClient");
                mHandler.sendMessage(mHandler.obtainMessage(STATUS_START_NOTIFY, map));
            } catch (Exception e) {
                Xlog.e(TAG, "Error notifyToClient", e);
            }
        }
        ...
        public void registerMessageCallback(IServiceMessageCallback callback) {
            try {
                Xlog.i(TAG, "registerMessageCallback, callback = " + callback.toString());
                mHandler.sendMessage(mHandler.obtainMessage(STATUS_MESSAGE_REG_CALLBACK, callback));
            } catch (Exception e) {
                Xlog.e(TAG, "Error registerMessageCallback", e);
            }
        }
        ...
        public void registerPresenterCallback(IServicePresenterCallback callback) {
            try {
                Xlog.i(TAG, "registerPresenterCallback, callback = " + callback.toString());
                mHandler.sendMessage(mHandler.obtainMessage(STATUS_PRESENTER_REG_CALLBACK, callback));
            } catch (Exception e) {
                Xlog.e(TAG, "Error registerPresenterCallback", e);
            }
        }
        ...
        public void unregisterMessageCallback(IServiceMessageCallback callback) {
            try {
                Xlog.i(TAG, "unregisterMessageCallback, callback = " + callback.toString());
                mHandler.sendMessage(mHandler.obtainMessage(STATUS_MESSAGE_UNREG_CALLBACK, callback));
            } catch (Exception e) {
                Xlog.e(TAG, "Error unregisterClientCallback", e);
            }
        }
        ...
        public void unregisterPresenterCallback(IServicePresenterCallback callback) {
            try {
                Xlog.i(TAG, "unregisterPresenterCallback, callback = " + callback.toString());
                mHandler.sendMessage(mHandler.obtainMessage(STATUS_PRESENTER_UNREG_CALLBACK, callback));
            } catch (Exception e) {
                Xlog.e(TAG, "Error unregisterClientCallback", e);
            }
        }
    ...
    private class RemoteServiceCallbackList <E extends IInterface> extends RemoteCallbackList<E> {
        @Override
        public void onCallbackDied(E callback) {
            Xlog.d(TAG, "onCallbackDied");
    ...
    private void notifyStatusToClient(HashMap<Integer, String> map) {
        ...
        Xlog.i(TAG, "notifyStatusToClient, name = " + name + " , status = " + status + " , time = " + time);
        int callbackNum = mMessageCallback.beginBroadcast();
        for (int i = 0; i < callbackNum; i++) {
            try {
                Xlog.i(TAG, "getBroadcastItem, index = " + i);
    ...
    private void notifyClientToStop() {
        Xlog.i(TAG, "notifyClientToStop");

        int callbackNum = mMessageCallback.beginBroadcast();
        for (int i = 0; i < callbackNum; i++) {
            try {
                Xlog.i(TAG, "notifyClientToStop, index = " + i);
    ...
    private void setClientStatus(int origin, int status) {
        Xlog.i(TAG, "setClientStatus, status = " + status);

        int callbackNum = mPresenterCallback.beginBroadcast();
        for (int i = 0; i < callbackNum; i++) {
            try {
                Xlog.i(TAG, "setClientStatus, index = " + i);
    ...
    private void onPresenterCallbackRegister(IServicePresenterCallback cb) {
        Xlog.i(TAG, "onPresenterCallbackRegister");
    ...
    private void onPresenterCallbackUnregister(IServicePresenterCallback cb) {
        Xlog.i(TAG, "onPresenterCallbackUnregister");
    ...
    private void onMessageCallbackRegister(IServiceMessageCallback cb) {
        Xlog.i(TAG, "onMessageCallbackRegister");
    ...
    private void onMessageCallbackUnregister(IServiceMessageCallback cb) {
        Xlog.i(TAG, "onMessageCallbackUnregister");
