...
public class MediaFolder {
    ...
    public static synchronized File getMediaDir(int type) {
        ...
        case Constants.MEDIA_TYPE_PICTURE:
            if (sPictureMediaDir == null) {
                sPictureMediaDir = new File(root.getAbsolutePath() + File.separator + MediaFolder.PICTURE_DIR_NAME);
        ...
        case Constants.MEDIA_TYPE_AUDIO:
            if (sAudioMediaDir == null) {
                sAudioMediaDir = new File(root.getAbsolutePath() + File.separator + MediaFolder.AUDIO_DIR_NAME);
        ...
        case Constants.MEDIA_TYPE_VIDEO:
            if (sVideoMediaDir == null) {
                sVideoMediaDir = new File(root.getAbsolutePath() + File.separator + MediaFolder.VIDEO_DIR_NAME);
        ...
        case Constants.MEDIA_TYPE_GEOLOC:
            if (sGeoLocMediaDir == null) {
                sGeoLocMediaDir = new File(root.getAbsolutePath() + File.separator + MediaFolder.GEOLOC_DIR_NAME);
        ...
        case Constants.MEDIA_TYPE_VCARD:
            if (sVcardMediaDir == null) {
                sVcardMediaDir = new File(root.getAbsolutePath() + File.separator + MediaFolder.VCARD_DIR_NAME);
    ...
    public static String generateMediaFileName(long mediaId, int type, String extension) {
        if (mediaId == Constants.INVALID) {
            ...
            File f = new File(getMediaDir(type).getAbsolutePath() + File.separator + TEMP_PREFIX + timestamp + extension);
            if (f.exists()) {
                int i = 0;
                for (; i < MAX_FILE_SEQUENCE; ++i) {
                    f = new File(getMediaDir(type).getAbsolutePath() + File.separator + TEMP_PREFIX + timestamp + "_" + i + extension);
                    if (!f.exists()) {
                        break;
                    }
                }
                if (i == MAX_FILE_SEQUENCE) {
                    Log.e(TAG, "MAX_FILE_SEQUENCE reached. Cannot find a temp filename in reasonable time.");
                    return null;
                } else {
