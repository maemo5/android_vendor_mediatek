...
public class AsyncUIPAMClient {
	...
	public interface Callback {
		...
		void reportGetMessageHistoryResult(long requestId, int resultCode, List<MessageContent> results);
	...
	private static class CallbackWrapper implements Callback {
		...
		@Override
		public void reportSearchResult(final long requestId, final int resultCode, final List<PublicAccount> results) {
		...
		@Override
		public void reportGetRecommendsResult(final long requestId, final int resultCode, final List<PublicAccount> results) {
			mHandler.post(new Runnable() {
				@Override
				public void run() {
		...
		@Override
		public void reportGetMessageHistoryResult(final long requestId, final int resultCode, final List<MessageContent> results) {
			mHandler.post(new Runnable() {
				@Override
				public void run() {
	...
	private static abstract class RequestedTask implements Runnable {
	...
	public long search(String keyword, int order, int pageSize, int pageNumber) {
		...
		final class SearchTask extends RequestedTask {
			...
			public SearchTask(long requestId, String keyword, int order, int pageSize, int pageNumber, Callback callback) {
			...
			@Override
			public void run() {
				Log.d(TAG, "search(" + mId + ", " + mKeyword + ", " + mOrder + ", " + mPageSize + ", " + mPageNumber + ")");
		...
		SearchTask task = new SearchTask(requestId, keyword, order, pageSize, pageNumber, mCallback);
	...
	public long getRecommends(int type, int pageSize, int pageNumber) {
		...
		final class GetRecommendsTask extends RequestedTask {
			...
			public GetRecommendsTask(long requestId, int type, int pageSize, int pageNumber, Callback callback) {
				super(requestId, callback);
				mType = type;
				mPageSize = pageSize;
				mPageNumber = pageNumber;
			...
			@Override
			public void run() {
				Log.d(TAG, "getRecommends(" + mId + ", " + mType + ", " + mPageSize + ", " + mPageNumber + ")");
				int result = ResultCode.SUCCESS;
				List<PublicAccount> accounts = null;
				try {
		...
		GetRecommendsTask task = new GetRecommendsTask(requestId, type, pageSize, pageNumber, mCallback);
		mWorkingTasks.put(requestId, task);
		mExecutorService.execute(task);
		return requestId;
	}

	public long getMessageHistory(String uuid, String timestamp, int order, int pageSize, int pageNumber) {
		long requestId = generateUuid();
		final class GetMessageHistoryTask extends RequestedTask {
			...
			public GetMessageHistoryTask(long requestId, String uuid, String timestamp, int order, int pageSize, int pageNumber, Callback callback) {
				super(requestId, callback);
				mUuid = uuid;
			...
			@Override
			public void run() {
				Log.d(TAG, "getMessageHistory(" + mId + ", " + mUuid + ", " + mTimestamp + ", " + mOrder + ", " + mPageSize + ", " + mPageNumber + ")");
				int result = ResultCode.SUCCESS;
				List<MessageContent> messages = null;
				try {
					messages = mClient.getMessageHistory(mUuid, mTimestamp, mOrder, mPageSize, mPageNumber);
				} catch (PAMException e) {
					result = e.resultCode;
				}
				if (mCallback != null) {
		...
		GetMessageHistoryTask task = new GetMessageHistoryTask(requestId, uuid, timestamp, order, pageSize, pageNumber, mCallback);
		mWorkingTasks.put(requestId, task);
		mExecutorService.execute(task);
		return requestId;
	}
	...
	public long complain(String uuid, int type, String reason, String data, String description) {
		...
		final class ComplainTask extends RequestedTask {
			...
			public ComplainTask(long requestId, String uuid, int type, String reason, String data, String description, Callback callback) {
				super(requestId, callback);
				mUuid = uuid;
				mType = type;
				mReason = reason;
			...
			@Override
			public void run() {
				Log.d(TAG, "complain(" + mId + ", " + mUuid + ", " + mType + ", " + mReason + ", " + mDescription + ")");
				int result = ResultCode.SUCCESS;
				try {
		...
		ComplainTask task = new ComplainTask(requestId, uuid, type, reason, data, description, mCallback);
		mWorkingTasks.put(requestId, task);
		mExecutorService.execute(task);
		return requestId;
	}
