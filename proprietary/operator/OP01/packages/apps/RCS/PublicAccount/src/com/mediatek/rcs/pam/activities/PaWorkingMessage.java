...
public class PaWorkingMessage {
    ...
    final String[] AudioSelectColumn = {
            Audio.Media.DATA
    };

    final String[] VideoSelectColumn = {
            "_data", "duration"
     };

    final String[] ImageSelectColumn = {
            "_data", "_size", "width", "height", "mime_type"
    };
    ...
    private int parseImageInfo() {
        ...
        cursor = mActivity.getContentResolver().query(mUri, ImageSelectColumn, null, null, null);
        if (null == cursor) {
    ...
    private int parseVideoInfo() {
        ...
        Cursor cursor = null;
        //parse path
        cursor = mActivity.getContentResolver().query(mUri,VideoSelectColumn, null, null, null);
        if (null == cursor) {
            mPath = mUri.getEncodedPath();
        } else if (0 == cursor.getCount()) {
    ...
    private int parseAudioInfo() {
        ...
        if (scheme.equals("file")) {
            ...
            Cursor c = mActivity.getContentResolver().query(mUri, AudioSelectColumn, null, null, null);
            c.moveToFirst();
    ...
    private int generateImageThumbnail() {
        ...
        Bitmap newBitmap = RcsMessageUtils.resizeImage(bitmap, 352, 288, true);        
        String thumbPath = RcsMessageUtils.getTempFilePath(
    ...
    private int generateVideoThumbnail() {
        ...
        try { 
            ...
            String path = RcsMessageUtils.getTempFilePath(
                    RcsMessageUtils.PA_FILE_PREFIX_VDO, RcsMessageUtils.PA_FILE_SUFFIX_JPG);
    ...
    public int zoomInImage() {
        ...
        long maxLen = 1024;
        byte[] img = RcsMessageUtils.resizeImg(mPath, maxLen);

        if (null == img) {
            return ERR_IMAGE_RESIZE_FAIL;
        }
        mZoomInPath = RcsMessageUtils.getTempFilePath(
                RcsMessageUtils.PA_FILE_PREFIX_IMG, RcsMessageUtils.PA_FILE_SUFFIX_JPG);
        try {
            RcsMessageUtils.nmsStream2File(img, mZoomInPath);
        } catch (Exception e) {
            e.printStackTrace();
            return ERR_IMAGE_RESIZE_FAIL;
        }
    ...
    public void saveDraft(final boolean isStopping) {
        ...
        if (mAccountId < 0) {
            throw new IllegalStateException("saveDraft() called with no account id");
        }
    ...
    private String readDraftTextMessage(long accountId) {
        ...
        Cursor c = cr.query(
            ...
            PAContract.MessageColumns.ACCOUNT_ID + "=? AND " + PAContract.MessageColumns.STATUS + "=?",
            new String[]{Long.toString(accountId), Long.toString(Constants.MESSAGE_STATUS_DRAFT)},
            null);
        ...
        if (c != null) {
            try {
                if (c.moveToFirst()) {
                    body = c.getString(c.getColumnIndex(PAContract.MessageColumns.TEXT));
                    haveDraft = true;
                }
            } finally {
                c.close();
    ...
    private void asyncUpdateDraftSmsMessage(final long accountId, final String contents,
    ...
    private void updateDraftTextMessage(final long accountId, String contents) {
       ...
       Log.d(TAG, "updateDraftSmsMessage accountId=" + accountId + ", contents=" + contents);
        ...
        try {
            c = cr.query(
                PAContract.MessageColumns.CONTENT_URI,
                new String[] {PAContract.MessageColumns._ID, MessageColumns.UUID},
                PAContract.MessageColumns.ACCOUNT_ID + "=? AND " + PAContract.MessageColumns.STATUS + "=?",
                new String[]{Long.toString(accountId), Long.toString(Constants.MESSAGE_STATUS_DRAFT)},
                null);
            ...
            if (c != null && c.getCount() > 0) {
                c.moveToFirst();
                msgId = c.getInt(c.getColumnIndex(PAContract.AccountColumns._ID));
                uuid = c.getString(c.getColumnIndex(PAContract.AccountColumns.UUID));
            }
        } finally {
            if (c != null) {
                c.close();
        ...
        if (msgId >= 0) {
            ...
            cr.update(
                    ...
                    new String[]{Integer.toString(msgId)});
        } else {
            ...
            cv.put(MessageColumns.TEXT, contents);
            cv.put(MessageColumns.DIRECTION, Constants.MESSAGE_DIRECTION_OUTGOING);
            ...
            msgId = Integer.parseInt(uri.getLastPathSegment());
    ...
    private void deleteDraftTextMessage(long accountId) {
        ...
        try {
            c = mContentResolver.query(
                PAContract.MessageColumns.CONTENT_URI,
                new String[] {PAContract.MessageColumns._ID},
                PAContract.MessageColumns.ACCOUNT_ID + "=? AND " + PAContract.MessageColumns.STATUS + "=?",
                new String[]{Long.toString(accountId), Long.toString(Constants.MESSAGE_STATUS_DRAFT)},
                null);
