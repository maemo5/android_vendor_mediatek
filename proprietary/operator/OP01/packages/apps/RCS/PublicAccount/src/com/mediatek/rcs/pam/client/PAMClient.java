...
public class PAMClient {
	...
	public interface Transmitter {
		public abstract Response sendRequest(String msgname, String content, boolean postOrGet) throws IOException;
	}
	...
	public void subscribe(String id) throws PAMException {
		...
		// build message
		String xmlMessage = mMessageBuilder.buildSubscribeRequest(id, PlatformManager.getInstance().getUserId(mContext));
		// send & receive
		Response response = sendRequest(MessageName.SUBSCRIBE, xmlMessage, true);
		Utils.throwIf(ResultCode.SYSTEM_ERROR_NETWORK, (response.result != HttpStatus.SC_OK));
		try {
			String uuid = mMessageParser.parseSubscribeMessage(new ByteArrayInputStream(response.content.getBytes("UTF-8")));
	...
	public void unsubscribe(String id) throws PAMException {
		...
		Utils.throwIf(ResultCode.PARAM_ERROR_MANDATORY_MISSING, TextUtils.isEmpty(id));
		// build message
		String xmlMessage = mMessageBuilder.buildUnsubscribeRequest(id, PlatformManager.getInstance().getUserId(mContext));
		// send & receive
		Response response = sendRequest(MessageName.UNSUBSCRIBE, xmlMessage, true);
		Utils.throwIf(ResultCode.SYSTEM_ERROR_NETWORK, (response.result != HttpStatus.SC_OK));
		try {
			String uuid = mMessageParser.parseUnsubscribeMessage(new ByteArrayInputStream(response.content.getBytes("UTF-8")));
			Utils.throwIf(ResultCode.PARAM_ERROR_MANDATORY_MISSING, !uuid.equals(id));
		} catch (UnsupportedEncodingException e) {
			throw new Error(e);
		}
	...
	public List<PublicAccount> getSubscribedList(int order, int pageSize, int pageNumber) throws PAMException {
		Log.d(TAG, "+getSubscribedList(" + order + ", " + pageSize + ", " + pageNumber + ")");
		// sanity check
		Utils.throwIf(ResultCode.PARAM_ERROR_INVALID_FORMAT,
				(pageSize <= 0) ||
				(pageNumber <= 0) ||
				(order != Constants.ORDER_BY_REVERSED_TIMESTAMP && order != Constants.ORDER_BY_NAME));
		// build message
		String xmlMessage = mMessageBuilder.buildGetSubscribedListRequest(order, pageSize, pageNumber, PlatformManager.getInstance().getUserId(mContext));
		// send & receive
		Response response = sendRequest(MessageName.GET_SUBSCRIBED_LIST, xmlMessage, true);
		Utils.throwIf(ResultCode.SYSTEM_ERROR_NETWORK, (response.result != HttpStatus.SC_OK));
		try {
			AccountsInfo result = mMessageParser.parseGetSubscribedListMessage(new ByteArrayInputStream(response.content.getBytes("UTF-8")));
	...
	public List<PublicAccount> search(String keyword, int order, int pageSize, int pageNumber) throws PAMException {
		...
		Utils.throwIf(ResultCode.PARAM_ERROR_INVALID_FORMAT,
				(pageSize <= 0) || (pageNumber <= 0) || (order != Constants.ORDER_BY_REVERSED_TIMESTAMP && order != Constants.ORDER_BY_NAME));
		...
		// build message
		String xmlMessage = mMessageBuilder.buildSearchRequest(keyword, order, pageSize, pageNumber, PlatformManager.getInstance().getUserId(mContext));
		// send & receive
		Response response = sendRequest(MessageName.SEARCH, xmlMessage, true);
		Utils.throwIf(ResultCode.SYSTEM_ERROR_NETWORK, (response.result != HttpStatus.SC_OK));
		try {
			AccountsInfo result = mMessageParser.parseSearchMessage(new ByteArrayInputStream(response.content.getBytes("UTF-8")));
			Log.d(TAG, "-search");
			return result.accounts;
		} catch (UnsupportedEncodingException e) {
			throw new Error(e);
	...
	public PublicAccount getDetails(String uuid, String timestamp) throws PAMException {
		...
		// build message
		String xmlMessage = mMessageBuilder.buildGetDetailsRequest(uuid, timestamp, PlatformManager.getInstance().getUserId(mContext));
		// send & receive
		Response response = sendRequest(MessageName.GET_DETAILS, xmlMessage, true);
		Utils.throwIf(ResultCode.SYSTEM_ERROR_NETWORK, (response.result != HttpStatus.SC_OK));
		try {
			PublicAccount result = mMessageParser.parseGetDetailsMessage(new ByteArrayInputStream(response.content.getBytes("UTF-8")));
	...
	public MenuInfo getMenu(String uuid, String timestamp) throws PAMException {
		...
		Utils.throwIf(ResultCode.PARAM_ERROR_MANDATORY_MISSING,
				(TextUtils.isEmpty(uuid)));
		// build message
		String xmlMessage = mMessageBuilder.buildGetMenuRequest(uuid, timestamp, PlatformManager.getInstance().getUserId(mContext));
		// send & receive
		Response response = sendRequest(MessageName.GET_MENU, xmlMessage, true);
		Utils.throwIf(ResultCode.SYSTEM_ERROR_NETWORK, (response.result != HttpStatus.SC_OK));
		try {
			MenuInfo result = mMessageParser.parseGetMenuMessage(new ByteArrayInputStream(response.content.getBytes("UTF-8")));
	...
	public List<MessageContent> getMessageHistory(String uuid, String timestamp, int order, int pageSize, int pageNumber) throws PAMException {
		Log.d(TAG, "+getMessageHistory(" + uuid + ", " + timestamp + ", " + order + ", " + pageSize + ", " + pageNumber + ")");
		// sanity check
		Utils.throwIf(ResultCode.PARAM_ERROR_MANDATORY_MISSING,
				(TextUtils.isEmpty(uuid) || TextUtils.isEmpty(timestamp)));
		Utils.throwIf(ResultCode.PARAM_ERROR_INVALID_FORMAT,
				(pageSize <= 0) || (pageNumber <= 0) || (order != Constants.ORDER_BY_TIMESTAMP_ASCENDING && order != Constants.ORDER_BY_TIMESTAMP_DESCENDING));
		// build message
		String xmlMessage = mMessageBuilder.buildGetMessageHistoryRequest(uuid, timestamp, order, pageSize, pageNumber, PlatformManager.getInstance().getUserId(mContext));
		// send & receive
		Response response = sendRequest(MessageName.GET_MESSAGE_HISTORY, xmlMessage, false);
		Utils.throwIf(ResultCode.SYSTEM_ERROR_NETWORK, (response.result != HttpStatus.SC_OK));
		try {
			MessageHistoryInfo result = mMessageParser.parseGetMessageHistoryMessage(new ByteArrayInputStream(response.content.getBytes("UTF-8")));
			Utils.throwIf(ResultCode.PARAM_ERROR_INVALID_FORMAT, !uuid.equals(result.uuid));
	...
	public int complain(String uuid, int type, String reason, String data, String description) throws PAMException {
		Log.d(TAG, "+complain(" + uuid + ", " + type + ", " + reason + ", " + data + ", " + description + ")");
		// sanity check
		Utils.throwIf(ResultCode.PARAM_ERROR_MANDATORY_MISSING,
				TextUtils.isEmpty(uuid));
		Utils.throwIf(ResultCode.PARAM_ERROR_MANDATORY_MISSING,
		...
		// build message
		String xmlMessage = mMessageBuilder.buildComplainRequest(uuid, type, reason, data, description, PlatformManager.getInstance().getUserId(mContext));
		// send & receive
		Response response = sendRequest(MessageName.COMPLAIN, xmlMessage, true);
		Utils.throwIf(ResultCode.SYSTEM_ERROR_NETWORK, (response.result != HttpStatus.SC_OK));
		try {
			ComplainInfo result = mMessageParser.parseComplainMessage(new ByteArrayInputStream(response.content.getBytes("UTF-8")));
			Utils.throwIf(ResultCode.PARAM_ERROR_INVALID_FORMAT, !uuid.equals(result.uuid));
			Log.d(TAG, "-complain");
	...
	public List<PublicAccount> getRecommends(int type, int pageSize, int pageNumber) throws PAMException {
		...
		// build message
		String xmlMessage = mMessageBuilder.buildGetRecommendsRequest(type, pageSize, pageNumber, PlatformManager.getInstance().getUserId(mContext));
		// send & receive
		Response response = sendRequest(MessageName.GET_RECOMMENDS, xmlMessage, true);
		Utils.throwIf(ResultCode.SYSTEM_ERROR_NETWORK, (response.result != HttpStatus.SC_OK));
		try {
			AccountsInfo result = mMessageParser.parseGetRecommendsMessage(new ByteArrayInputStream(response.content.getBytes("UTF-8")));
	...
	public int setAcceptStatus(String uuid, int acceptStatus) throws PAMException {
		...
		// build message
		String xmlMessage = mMessageBuilder.buildSetAcceptStatusRequest(uuid, acceptStatus, PlatformManager.getInstance().getUserId(mContext));
		// send & receive
		Response response = sendRequest(MessageName.GET_RECOMMENDS, xmlMessage, true);
		Utils.throwIf(ResultCode.SYSTEM_ERROR_NETWORK, (response.result != HttpStatus.SC_OK));
		try {
			SetAcceptStatusInfo result = mMessageParser.parseSetAcceptStatusMessage(new ByteArrayInputStream(response.content.getBytes("UTF-8")));
	...
	private Response sendRequest(String msgname, String content, boolean postOrGet) throws PAMException {
		try {
