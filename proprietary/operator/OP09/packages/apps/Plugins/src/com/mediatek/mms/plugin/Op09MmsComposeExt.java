...
/// M: ---------------new OP09Plugin Refactory-------------------------
    ...
    private Map<Integer, SubscriptionInfo> mSiminfoList = null;
    ...
    public void setDualSendButtonType(final ViewOnClickListener btnListener) {
        ...
        mSiminfoList = MessageUtils.getSimInfoFromCache(mContext);
        int simCardSize = 0;
        simCardSize = SubscriptionManager.from(mContext).getActiveSubscriptionInfoCount();
        ...
        if (simCardSize == 0) {
            ...
            int subId = SubscriptionManager.from(mContext).getActiveSubscriptionIdList()[0];
            /// M: For get disable button for slot @{
            resIds = mmsUtilExt.getSendButtonResourceIdBySubId(mContext, subId, false);
            if (resIds != null) {
            ...
            if (mButtonBig.isEnabled()) {
                resIds = mmsUtilExt.getSendButtonResourceIdBySubId(mContext, subId, true);
            }
            if (resIds == null) {
                return;
            }
            ...
            if (defaultSubId < 0) {
                defaultSubId = MessageUtils.getFirstSimInfoBySlotId(mContext, 0)
                        .getSubscriptionId();
            }
            Xlog.d(TAG, "default SIM Sub Id:" + defaultSubId);
            /// M: For get disable button for slot @{
            resIds = mmsUtilExt.getSendButtonResourceIdBySubId(mContext, defaultSubId, false);
            if (resIds != null) {
                mButtonDrawable[0] = (Drawable) resIds[0][0];
                mButtonDrawable[1] = (Drawable) resIds[1][0];
            }
            /// @}
            if (mButtonBig.isEnabled()) {
                resIds = mmsUtilExt.getSendButtonResourceIdBySubId(mContext, defaultSubId,
        ...
        View.OnClickListener buttonClickListener = new View.OnClickListener() {
            public void onClick(View v) {
                int mSendSubIdForDualBtn = -1;
                if (v == mButtonBig) {
                    SubscriptionInfo sir = MessageUtils.getFirstSimInfoBySlotId(mContext,
                        mButtonSlotIds[0]);
                    mSendSubIdForDualBtn = sir.getSubscriptionId();
                } else if (v == mButtonSmall) {
                    SubscriptionInfo sir = MessageUtils.getFirstSimInfoBySlotId(mContext,
                        mButtonSlotIds[1]);
                    mSendSubIdForDualBtn = sir.getSubscriptionId();
                }
                btnListener.setSelectedSubId(mSendSubIdForDualBtn);
    ...
    public void updateDualSendButtonStatue(boolean enable, boolean isMms) {
        Xlog.d(TAG, "updateDualSendButtonStatue  enable: " + enable + " isMms:" + isMms);
        if (mButtonBig == null || mButtonSmall == null) {
            return;
        }
        Map<Integer, SubscriptionInfo> simInfoList = MessageUtils.getSimInfoFromCache(mContext);
        int simCount = 0;
        if (simInfoList == null) {
            simCount = 0;
        } else {
        ...
        if (bigEnable != enable && mButtonSlotIds != null && mButtonColors != null) {
            ...
            if (enable) {
                drawable = mmsUtilsExt.getActivatedButtonIconBySlotId(mButtonSlotIds[0], false,
                    mButtonColors[0]);
            } else {
        ...
        if (smallEnable != enable && mButtonSlotIds != null && mButtonColors != null) {
            Drawable drawable = null;
            if (enable) {
                drawable = mmsUtilsExt.getActivatedButtonIconBySlotId(mButtonSlotIds[1], true,
                    mButtonColors[1]);
            } else {
                if (mButtonDrawable != null) {
