...
public class RegisterService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (action.equalsIgnoreCase(Const.ACTION_BOOT_COMPLETED) ||
            ...
            if (action.equalsIgnoreCase(Const.ACTION_RETRY)) {
                int[] subId = SubscriptionManager.getSubId(0);
                if (subId != null && subId[0] > 0) {
                    Log.d(SUB_TAG, "retry registerPhoneStateListener, subId: " + subId[0]);
                    registerPhoneStateListener(subId[0]);
                } else {
                    Log.e(SUB_TAG, "retry registerPhoneStateListener, subId invalid:"
            ...
            int[] subId = SubscriptionManager.getSubId(0);
            if (subId != null && subId[0] > 0) {
                Log.d(SUB_TAG, "onStartCommand(), subId: " + subId[0]);
                registerPhoneStateListener(subId[0]);
            } else {
                Log.e(SUB_TAG, "onStartCommand(), subId invalid:"
                        + (subId == null ? "null" : subId[0]));
                registerPhoneStateListener(SubscriptionManager.getDefaultSubId());
            }
        } else {
